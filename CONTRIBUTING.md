...
CI selects frontend, browser, Rust, bridge, and release-validation checks from the changed paths. A release-bearing input validates metadata and packages the Windows validation artifact before `check` succeeds; only verified-`main` Release can publish it. Do not weaken tests to make a gate pass. Run `pnpm exec playwright install chromium` once after `pnpm install` so smoke can run.
...
The workflow overview is in [.codex/README.md](.codex/README.md). [AGENTS.md](AGENTS.md) owns standing repository rules, and the installed `workflow-core` skill owns shared lifecycle and model-routing policy. Development is trunk-based: short-lived branches merge to `main` frequently, and every commit is atomic.
...
Resolve conflicts manually. Review changes to `.codex/`, `scripts/`, and `AGENTS.md` before you merge. Update the globally installed workflow skills separately when the template changes them.
