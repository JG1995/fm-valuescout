...
CI selects frontend, browser, Rust, bridge, and release-validation jobs from changed paths. A release-bearing input also validates metadata and packages the Windows validation artifact before the required `check` status succeeds. Only verified `main` can publish.
...
- `.pi/settings.json` intentionally leaves machine package and preference settings global; and
- `.pi/skills/create-pr/SKILL.md` owns this repository's pull-request preparation procedure.
...
3. Invoke `/skill:workflow-deliver-feature <ledger>` once to execute the recorded commits, PRs, close-out, and release outcome.

The delivery workflow stops when authority changes, a decision or replan is required, validation fails, correction limits are exhausted, a PR head is stale, or release verification fails. Narrower workflow skills remain available for manual recovery; they are not required between normal delivery phases.
...
Every human-authored pull request uses [.pi/skills/create-pr/SKILL.md](.pi/skills/create-pr/SKILL.md) and [.github/pull_request_template.md](.github/pull_request_template.md). Record exactly one release intent. Release-bearing PRs must also satisfy [.wiki/notes/early-alpha-release-runbook.md](.wiki/notes/early-alpha-release-runbook.md).
