...
pub fn get_tactic(conn: &Connection, save_id: i64) -> Result<PlannerTactic, String> {
    ...
    let tactic_exists: bool = conn
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM planner_tactics WHERE save_id = ?1)",
            params![save_id],
            |row| row.get(0),
        )
...
pub fn save_tactic(conn: &Connection, save_id: i64, tactic: &PlannerTactic) -> Result<(), String> {
    ...
    let tx = conn
        ...
        .map_err(|error| error.to_string())?;
    tx.execute(
        "INSERT INTO planner_tactics (save_id, ip_weight)
         VALUES (?1, ?2)
         ON CONFLICT(save_id) DO UPDATE SET ip_weight = excluded.ip_weight",
        params![save_id, tactic.ip_weight],
    )
    .map_err(|error| error.to_string())?;
    tx.execute(
...
fn load_tactic(conn: &Connection, save_id: i64) -> Result<PlannerTactic, String> {
    let ip_weight = conn
        .query_row(
            "SELECT ip_weight FROM planner_tactics WHERE save_id = ?1",
            params![save_id],
            |row| row.get::<_, f64>(0),
        )
        .map_err(|error| error.to_string())?;
    let mut statement = conn
        .prepare(
            "SELECT lane_id, ip_position, ip_role_id, oop_position, oop_role_id
    ...
    let lanes = statement
        .query_map(params![save_id], |row| {
            Ok(TacticLane {
                lane_id: row.get(0)?,
                ip_position: row.get(1)?,
                ip_role_id: row.get(2)?,
                oop_position: row.get(3)?,
                oop_role_id: row.get(4)?,
            })
        })
        .map_err(|error| error.to_string())?
    ...
    let tactic = PlannerTactic { ip_weight, lanes };
    validate_tactic(&tactic)?;
    Ok(tactic)
}
...
fn validate_tactic(tactic: &PlannerTactic) -> Result<(), String> {
    if !tactic.ip_weight.is_finite() || !(0.0..=1.0).contains(&tactic.ip_weight) {
        return Err("IP weight must be between 0 and 1".to_string());
    }
...
mod tests {
    ...
    fn default_tactic_has_eleven_linked_lanes_and_expected_shapes() {
        ...
        assert_eq!(tactic.ip_weight, 0.5);
    ...
    fn tactic_is_save_scoped_and_reloads_after_update() {
        ...
        assert_eq!(reloaded.ip_weight, 0.7);
