...
pub const GRAPHICS_ROOT_V46_SQL: &str = "
CREATE TABLE IF NOT EXISTS graphics_settings (
    id INTEGER PRIMARY KEY CHECK (id = 1),
    root TEXT
);
INSERT OR IGNORE INTO graphics_settings (id, root) VALUES (1, NULL);
";
...
pub const TACTIC_RESET_SQL: &str = "
DELETE FROM planner_tactic_lanes;
";

pub const CLUB_UIDS_V44_SQL: &str = "
ALTER TABLE players ADD COLUMN current_club_uid INTEGER CHECK (current_club_uid IS NULL OR current_club_uid > 0);
ALTER TABLE players ADD COLUMN parent_club_uid INTEGER CHECK (parent_club_uid IS NULL OR parent_club_uid > 0);
ALTER TABLE staff ADD COLUMN club_uid INTEGER CHECK (club_uid IS NULL OR club_uid > 0);
ALTER TABLE snapshots ADD COLUMN manager_club_uid INTEGER CHECK (manager_club_uid IS NULL OR manager_club_uid > 0);
";

pub const PLANNER_STRING_NAMES_SQL: &str = "
ALTER TABLE planner_strings
    ADD COLUMN display_name TEXT NOT NULL DEFAULT '';

UPDATE planner_strings
SET display_name = CAST(string_order + 1 AS TEXT) || CASE
    WHEN (string_order + 1) % 100 IN (11, 12, 13) THEN 'th string'
    WHEN (string_order + 1) % 10 = 1 THEN 'st string'
    WHEN (string_order + 1) % 10 = 2 THEN 'nd string'
    WHEN (string_order + 1) % 10 = 3 THEN 'rd string'
    ELSE 'th string'
END;
";
...
pub fn all() -> &'static [Migration] {
    &[
        ...
        Migration {
            version: 42,
            description: "reset_planner_tactic_lanes",
            sql: TACTIC_RESET_SQL,
        },
        Migration {
            version: 43,
            description: "add_planner_string_display_names",
            sql: PLANNER_STRING_NAMES_SQL,
        },
        Migration {
            version: 44,
        ...
        Migration { version: 46, description: "create_graphics_settings", sql: GRAPHICS_ROOT_V46_SQL },
    ]
}
...
mod tests {
    ...
    #[test]
    fn migrates_populated_v41_by_resetting_only_tactic_lanes() {
        use crate::features::planner::tactic;

        let temp_dir = tempfile::tempdir().expect("temp dir");
        let conn = Connection::open(temp_dir.path().join("v41-tactic-reset.db")).expect("open db");
        conn.pragma_update(None, "foreign_keys", true)
            .expect("enable foreign keys");
        for migration in all().iter().filter(|migration| migration.version <= 41) {
            conn.execute_batch(migration.sql)
                .expect("apply v41 migration");
            conn.pragma_update(None, "user_version", migration.version)
                .expect("set v41 version");
        }
        conn.execute_batch(
            "INSERT INTO saves (id, name, is_active) VALUES
                 (1, 'First save', 1),
                 (2, 'Second save', 0);",
        )
        .expect("insert saves");
        let seeded = tactic::default_tactic();
        for save_id in [1_i64, 2] {
            for (order, lane) in seeded.lanes.iter().enumerate() {
                conn.execute(
                    ...
                    params![
                        ...
                        order as i64,
                        lane.lane_id,
                        if save_id == 1 { 0.8 } else { 0.3 },
                        if order < 2 {
                            Some(order as u8 + save_id as u8)
                        } else {
                            None
                        },
                        if save_id == 1 { "left" } else { "both" },
                        "strict",
                        lane.ip_position,
                        lane.ip_role_id,
                        lane.oop_position,
                ...
                .expect("seed divergent tactic lane");
            }
        }
        conn.execute_batch(
            "INSERT INTO planner_teams (save_id, team, display_name) VALUES
                 (1, 'senior', 'Senior'),
                 (2, 'youth', 'Academy');
             INSERT INTO planner_strings (id, save_id, team, string_order) VALUES
                 (1, 1, 'senior', 0),
                 (2, 2, 'youth', 0);
             INSERT INTO planner_assignments
                 (save_id, string_id, lane_id, player_uid, last_known_name) VALUES
                 (1, 1, 'goalkeeper', 7, 'First Keeper'),
                 (2, 2, 'centre_forward', 9, 'Second Striker');",
        )
        .expect("seed v41 planner state");

        apply(&conn).expect("migrate v41 database");
        ...
        assert_eq!(
            ...
            .expect("clear tactic rows"),
            0
        );
        for save_id in [1, 2] {
            assert_eq!(
                tactic::get_tactic(&conn, save_id).expect("lazily reseed default tactic"),
                tactic::default_tactic(),
                "save {save_id} reseeds the normal defaults"
            );
        }
        let mut recreated = tactic::get_tactic(&conn, 1).expect("read reseeded tactic");
        recreated.lanes[0].ip_weight = 0.7;
        tactic::save_tactic(&conn, 1, &recreated).expect("save recreated custom tactic");
        ...
        let assignments = conn
            .prepare(
                "SELECT lane_id, player_uid, last_known_name
                 FROM planner_assignments ORDER BY save_id",
            )
            .expect("prepare preserved assignments")
            .query_map([], |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, i64>(1)?,
            ...
            .expect("read preserved assignments")
            .collect::<Result<Vec<_>, _>>()
            .expect("collect preserved assignments");
        assert_eq!(
            assignments,
            [
                ("goalkeeper".to_string(), 7, "First Keeper".to_string()),
                (
                    "centre_forward".to_string(),
                    9,
                    "Second Striker".to_string()
                ),
            ]
        );
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM planner_strings", [], |row| {
                row.get::<_, i64>(0)
            })
            .expect("retain planner strings"),
            2
        );
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM planner_teams", [], |row| {
                row.get::<_, i64>(0)
            })
            .expect("retain planner teams"),
            2
    ...
    #[test]
    fn migrates_v42_strings_to_ordinal_display_names() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        let conn = Connection::open(temp_dir.path().join("v42-string-names.db")).expect("open db");
        conn.pragma_update(None, "foreign_keys", true)
            .expect("enable foreign keys");
        for migration in all().iter().filter(|migration| migration.version <= 42) {
            conn.execute_batch(migration.sql)
                .expect("apply v42 migration");
            conn.pragma_update(None, "user_version", migration.version)
        ...
        .expect("insert teams");
        for order in 0..13_i64 {
            conn.execute(
                "INSERT INTO planner_strings (id, save_id, team, string_order)
                 VALUES (?1, 1, 'senior', ?2)",
                params![order + 1, order],
            )
            .expect("insert senior string");
        }
        conn.execute_batch(
            "INSERT INTO planner_strings (id, save_id, team, string_order) VALUES
                 (101, 1, 'reserves', 0),
             ...
             INSERT INTO planner_assignments
                 (save_id, string_id, lane_id, player_uid, last_known_name) VALUES
                 (1, 1, 'goalkeeper', 7, 'First Keeper'),
                 (1, 12, 'centre_forward', 9, 'Twelfth Striker'),
                 (1, 101, 'goalkeeper', 11, 'Reserve Keeper');",
        )
        .expect("seed v42 planner strings");

        apply(&conn).expect("migrate v42 database");
        ...
        let senior_labels = conn
            .prepare(
                "SELECT string_order, display_name FROM planner_strings
                 WHERE save_id = 1 AND team = 'senior' ORDER BY string_order",
            )
            .expect("prepare senior labels")
            .query_map([], |row| {
                Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?))
            })
            .expect("read senior labels")
            .collect::<Result<Vec<_>, _>>()
            .expect("collect senior labels");
        assert_eq!(
            senior_labels
                .iter()
                .map(|(order, label)| (*order, label.as_str()))
                .collect::<Vec<_>>(),
            [
                (0, "1st string"),
                (1, "2nd string"),
        ...
        let reserve_labels = conn
            .prepare(
                "SELECT string_order, display_name FROM planner_strings
                 WHERE save_id = 1 AND team = 'reserves' ORDER BY string_order",
            )
            .expect("prepare reserve labels")
            .query_map([], |row| {
                Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?))
            })
            .expect("read reserve labels")
            .collect::<Result<Vec<_>, _>>()
            .expect("collect reserve labels");
        assert_eq!(
            reserve_labels
                .iter()
                .map(|(order, label)| (*order, label.as_str()))
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM planner_strings
                 WHERE trim(display_name) = ''",
                [],
                |row| row.get::<_, i64>(0)
            )
            .expect("count blank display names"),
            0
        );
        let assignments = conn
            .prepare(
                "SELECT string_id, lane_id, player_uid, last_known_name
                 FROM planner_assignments ORDER BY player_uid",
            )
            .expect("prepare preserved assignments")
            .query_map([], |row| {
                Ok((
        ...
        assert_eq!(
            assignments,
            [
                (1, "goalkeeper".to_string(), 7, "First Keeper".to_string()),
                (
                    12,
                    "centre_forward".to_string(),
                    9,
                    "Twelfth Striker".to_string()
                ),
                (
                    101,
                    "goalkeeper".to_string(),
                    11,
                    "Reserve Keeper".to_string()
                ),
            ]
        );
        assert_eq!(
    ...
    fn migrates_populated_v28_to_managed_club_settings() {
        ...
        assert!(
            !tactic_lanes_before.is_empty(),
            "seeded tactic lanes must exist before the v42 reset"
        );
        assert!(
            tactic_lanes_after.is_empty(),
            "migration v42 resets stored tactics to the normal defaults"
        );
    ...
    fn migrates_v8_tactic_lanes_with_default_rank_and_foot_preferences() {
        ...
        // Migration v42 intentionally clears tactic lanes on upgrade, so the
        // rank and foot defaults are proved through v10 instead of the full chain.
        for migration in all()
            .iter()
            .filter(|migration| migration.version == 9 || migration.version == 10)
        {
            conn.execute_batch(migration.sql)
                .expect("apply rank and foot preference migrations");
            conn.pragma_update(None, "user_version", migration.version)
                .expect("set migration version");
        }
    ...
    fn migrates_snapshot_schema_from_every_prior_version() {
        ...
        for legacy_version in 1..24 {
            ...
            let version: i32 = conn
                .pragma_query_value(None, "user_version", |row| row.get(0))
                .expect("read user version");
            assert_eq!(version, 46, "legacy version {legacy_version}");
            assert_eq!(
    ...
    #[test]
    fn v44_preserves_legacy_rows_with_null_club_uids() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        let conn =
            Connection::open(temp_dir.path().join("v44-null-preservation.db")).expect("open db");
        conn.pragma_update(None, "foreign_keys", true)
            .expect("enable foreign keys");
        for migration in all().iter().filter(|migration| migration.version <= 43) {
            conn.execute_batch(migration.sql)
                .expect("apply v43 migration");
            conn.pragma_update(None, "user_version", migration.version)
                .expect("set v43 version");
        }
        conn.execute_batch(
            "INSERT INTO saves (id, name, is_active) VALUES (1, 'Legacy', 1);
             INSERT INTO snapshots (
                 id, save_id, is_current, schema_version, generated_at_utc,
                 game_version, supported_game_version, bridge_version, protocol_version,
                 game_date_source, game_date_basis, player_database_scope,
                 scan_truncated, player_count, staff_count
             ) VALUES (1, 1, 1, 8, '2026-01-01T00:00:00Z', '26.3.2', '26.3',
                       '0.1.0', 1, 'unknown', 'unknown', 'men', 0, 0, 0);",
        )
        ...
        conn.execute_batch(
            "INSERT INTO players (
                 ...
                 nationalities_json, preferred_foot, positions_json, attributes_json,
                 hidden_attributes_json, personality_json
             ) VALUES (1, 7, 100, 150, 'Legacy Player', 2000, 1, '[]', 'unknown', '{}', '{}', '{}', '{}');
             INSERT INTO staff (
                 snapshot_id, uid, nationalities_json, gender, ca, pa, staff_attributes_json
             ) VALUES (1, 8, '[]', 'unknown', 100, 150, '{}');",
        )
        .expect("insert legacy player and staff");

        apply(&conn).expect("apply v44 migration");

        let manager_club_uid: Option<i64> = conn
            .query_row(
                "SELECT manager_club_uid FROM snapshots WHERE id = 1",
                [],
                |row| row.get(0),
            )
            .expect("read preserved legacy snapshot");
        assert_eq!(manager_club_uid, None);
        let player_uids: (Option<i64>, Option<i64>) = conn
            .query_row(
                "SELECT current_club_uid, parent_club_uid FROM players WHERE snapshot_id = 1 AND uid = 7",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .expect("read preserved legacy player");
        assert_eq!(player_uids, (None, None));
        let staff_club_uid: Option<i64> = conn
            .query_row(
                "SELECT club_uid FROM staff WHERE snapshot_id = 1 AND uid = 8",
                [],
                |row| row.get(0),
            )
            .expect("read preserved legacy staff");
        assert_eq!(staff_club_uid, None);
    }
    ...
    fn registers_monotonic_migrations() {
        ...
        assert_eq!(migrations[41].description, "reset_planner_tactic_lanes");
        assert_eq!(migrations[41].sql, TACTIC_RESET_SQL);
        assert_eq!(migrations[42].version, 43);
        assert_eq!(
            migrations[42].description,
            "add_planner_string_display_names"
        );
        assert_eq!(migrations[42].sql, PLANNER_STRING_NAMES_SQL);
    }
    ...
    fn opening_fresh_db_applies_planner_depth_schema() {
        ...
        assert_eq!(
            ...
            ["id", "save_id", "team", "string_order", "display_name"]
        );
        assert_eq!(
    ...
    #[test]
    fn v46_fresh_schema_has_exactly_one_empty_graphics_root() {
        let conn = Connection::open_in_memory().expect("open database");
        conn.execute_batch(GRAPHICS_ROOT_V46_SQL)
            .expect("apply v46");
        let rows: Vec<(i64, Option<String>)> = conn
            .prepare("SELECT id, root FROM graphics_settings ORDER BY id")
            .expect("prepare graphics rows")
            .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))
            .expect("query graphics rows")
            .collect::<Result<_, _>>()
            .expect("read graphics rows");
        assert_eq!(rows, vec![(1, None)]);
    }

    #[test]
    fn v46_upgrade_preserves_real_v45_data_and_adds_one_graphics_row() {
        let conn = Connection::open_in_memory().expect("open database");
        for migration in all().iter().filter(|m| m.version <= 45) {
            conn.execute_batch(migration.sql)
                .expect("apply v45 migration");
            conn.pragma_update(None, "user_version", migration.version)
                .expect("record v45 migration");
        }
        conn.execute_batch(
            "INSERT INTO saves (id, name, is_active) VALUES (1, 'Existing', 1);
             INSERT INTO managed_club_settings (save_id, club_name) VALUES (1, 'Existing FC');",
        )
        .expect("seed representative v45 data");

        apply(&conn).expect("apply v46");

        let club: String = conn
            .query_row(
                "SELECT club_name FROM managed_club_settings WHERE save_id=1",
                [],
                |r| r.get(0),
            )
            .expect("read preserved v45 data");
        assert_eq!(club, "Existing FC");
        let rows: Vec<(i64, Option<String>)> = conn
            .prepare("SELECT id, root FROM graphics_settings ORDER BY id")
            .expect("prepare graphics rows")
            .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))
