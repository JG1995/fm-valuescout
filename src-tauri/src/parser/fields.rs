...
pub fn parse_transfer_value(raw: &str) -> (TransferValue, Option<String>) {
    ...
    match parts.len() {
        ...
        2 => {
            let low_val = parse_money_value(strip_currency(parts[0].trim()));
            let high_val = parse_money_value(strip_currency(parts[1].trim()));
...
/// Strip leading currency symbols from a monetary string part.
fn strip_currency(s: &str) -> &str {
    let s = s.trim();
...
pub fn parse_date(raw: &str) -> Option<String> {
    ...
    // Manual split to handle both 4-digit and 2-digit years correctly
...
mod tests {
    ...
    fn parse_second_nationality() {
        let nat = super::parse_second_nationality("Ireland").unwrap();
    ...
    #[test]
    fn parse_distance() {
        assert_eq!(super::parse_distance("312.7km"), Some(312.7));
        assert_eq!(super::parse_distance("66.2km"), Some(66.2));
    }
