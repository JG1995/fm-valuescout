...
${ARGUMENTS:-Use the active commit from the active feature ledger, or the single coherent outcome the developer named.}
...
Follow `.cursor/commands/build.md` in full through implementation, format, gate, and ledger updates for the active commit.

Do **not** stop for manual `/checkpoint` at the end of build — continue into Phase 2 in the **same command invocation** (do not wait for the developer between phases).
...
1. Commit the staged content locally with the assessed message from the final checkpoint (Conventional Commits per `.cursor/skills/conventional-commits/SKILL.md`). No approval wait.
2. **Immediately** advance the active feature ledger per `.cursor/commands/checkpoint.md` step **15** — real hash, **Completed work** row, next commit `Active`, refresh **Active work**.
...
Same as `/build` plus `.cursor/commands/checkpoint.md` and `.cursor/commands/fix.md` for loop phases.

**Coding standards:** Read `.cursor/skills/coding-standards/SKILL.md`, `references/universal.md`, and `references/testing.md` when the commit adds or changes tests. Load matching stack references when `ARCHITECTURE.md` or touched files apply.

**Minimalism:** Run the decision ladder in `.cursor/skills/minimalism/SKILL.md` during build and fix phases.
...
| Command | Scope |
| --- | --- |
| `/build` | Implement active commit; stop for manual `/checkpoint` |
| `/build-loop` | Implement active commit + automated checkpoint/fix loop + auto-commit on success |
| `/fix` | Correct delegated review findings; stop for manual `/checkpoint` unless under `/build-loop` |
| `/checkpoint` | Stage, review, present; commit only when developer approves |

Do not use `/build-loop` for `/finish-feature`, documentation reconciliation, or multi-commit feature runs.
