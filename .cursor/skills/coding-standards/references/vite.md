...
Add new dependencies only through the minimalism ladder in `.cursor/skills/minimalism/SKILL.md`.
...
- **`format`** runs **`biome check --write`** on the project or forwarded paths, then **`cargo fmt`** in `src-tauri/`. Path args apply to Biome only. Use before `/build` checkpoint staging — not in CI or Husky.
...
Paths from this file: `.cursor/skills/coding-standards/references/vite.md` → `.wiki/decisions/`.
