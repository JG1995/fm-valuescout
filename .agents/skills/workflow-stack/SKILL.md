...
1. Read `.agents/skills/project-strategy/SKILL.md` and follow its workflow, hard gates, and output contracts.
...
Apply developer-supplied constraints after wiki reads; they refine or override product notes. If none are supplied, base the recommendation on CONCEPT.md and inspected evidence.
...
Do not dispatch a planning subagent. `$workflow-stack` chooses stack and target architecture from product notes; `$workflow-plan-feature` plans delivery inside that agreed direction.
...
Do not start `$workflow-build` or feature implementation unless the developer asks.
