...
Recallium is wired in `.codex/config.toml` (remote MCP server). Use Recallium MCP tools directly — not this skill file — for search and storage.

**Authority:** `AGENTS.md` § Recallium project and § Recallium memory override generic Recallium defaults. `AGENTS.md` is always applied. Wiki and Git own formal plans; Recallium holds context that explains them.
...
| --- | --- | --- |
| `$workflow-stack` | Prior stack or architecture decisions | Approved stack choices |
| `$workflow-roadmap` | Prior sequencing or dependency decisions | Approved sequence rationale |
| `$workflow-plan-feature` | Prior work on this feature or area | Durable planning context |
| `$workflow-build` | Unfamiliar conventions for the commit | Durable progress or constraints |
| `$workflow-build-loop` | Same as `$workflow-build`; checkpoint context each loop pass | Durable progress or remediation not in ledger |
...
| `$workflow-security-audit` | Prior security decisions or incidents | Do not save |
