...
4. Obtain separate explicit approval to push unless this procedure runs inside an explicitly invoked `$workflow-deliver-feature` session whose unchanged Delivery fingerprint covers the exact branch, base, commits, and PR. Re-check the branch, clean worktree, unchanged `HEAD`, and `gh auth status`, then push without force.
