...
Read `.agents/skills/technical-writing/SKILL.md` before writing prose in audit reports, wiki updates, or comments meant for humans.
...
| `references/testing.md` | Every `$workflow-build`, `$workflow-build-loop`, `$workflow-fix`, `$workflow-checkpoint`, and code review that touches or adds tests |
...
Other skills in `.agents/skills/` (e.g. `ui-design`) may add domain rules. When their description matches the work, read them alongside this skill.

## When to apply

| Phase | Requirement |
| --- | --- |
| `$workflow-build`, `$workflow-build-loop`, `$workflow-fix` | Read this skill + `universal.md` + `testing.md` when the commit adds or changes tests + matching refs before editing production code or tests |
| `$workflow-checkpoint`, `$workflow-review`, `reviewer` | Read before judging conventions in the diff |
| `$workflow-plan-feature` | Optional for delivery planning; required when the plan names concrete patterns |

## Before you add code

1. Read `references/universal.md`.
2. Read matching stack and project references from the table above.
3. Run the **intentional minimalism** decision ladder in `.agents/skills/minimalism/SKILL.md` before adding dependencies, files, classes, or abstractions. Minimalism governs scope; universal governs shape.

State which ladder rung you stopped at in `$workflow-build` when the decision matters for the commit.
...
Flag violations of loaded references as **HIGH** when they break project conventions or layer boundaries, or **MEDIUM** when they harm readability without functional impact. Functional bugs remain the reviewer's first priority.
