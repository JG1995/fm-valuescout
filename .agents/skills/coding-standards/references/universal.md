...
When a stack or project reference (`references/nextjs.md`, `references/rust.md`, `references/project.md`, or a matching skill under `.agents/skills/`) defines an idiomatic pattern that conflicts with a rule below, **the stack or project rule wins**.
...
Prose in wiki, prompts, and user-facing docs: follow `.agents/skills/technical-writing/SKILL.md`.
...
Before you add a dependency, function, class, or file, run the ladder in `.agents/skills/minimalism/SKILL.md`. Stop at the first rung that solves the problem:
