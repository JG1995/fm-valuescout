...
- The pinned `tauri-plugin-mcp-bridge` and `@hypothesi/tauri-mcp-server` 0.12.0 integration gives a trusted Codex task upstream WebView inspection, interaction, resize, screenshots, logs, JavaScript, and IPC access.
...
- The upstream MCP server owns session state. In version 0.12.0, its advertised IPC command executor does not dispatch application-defined commands; a necessary read-only product IPC proof uses `window.__TAURI__.core.invoke(...)` through the trusted WebView JavaScript tool.
- In WSL UI-agent mode, Vite console forwarding is disabled to avoid an upstream locale-error serialization crash. MCP frontend logs and launcher output remain the diagnostics sources.

## Important decisions

- Use the pinned upstream integration directly. The repository does not maintain a custom MCP server, WebDriver layer, session protocol, or filtering wrapper.
...
- Start a fresh Codex task after dependency installation or a project MCP configuration change, then start the launcher in another terminal and connect with upstream `driver_session`.
...
- Run a fresh-task upstream control proof that covers HMR, restart, both viewports, accessibility, and log forwarding.
