...
| PR 1 — Scale local graphics delivery | Commit 11 — Batch config source validation | `80367d689e515bf7ee61073ac19b26f3dc484dd4` | Grouped clean-EOF candidates by relative source directory, enumerated each capability once with no-follow regular-file checks, and folded valid probes in original record order. | Graphics index tests passed; `./scripts/dev check-rust`, `./scripts/dev check`, `git diff --cached --check`, and LSP passed. | Pass | Clear | 2 | Representative calibration exposed quadratic entry-to-probe matching; bounded correction `409d53a7c59f04b5fe0c944673894471c1f9f268` replaced it with reviewed sorted binary-range matching. PR CI repair round 1 narrowed its test-only source helper to Unix after Windows correctly reported dead code; both corrections preserved the packet contract. |
