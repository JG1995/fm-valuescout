...
> **Authority:** These files own pre-implementation feature intent and behavioral detail for sequencing (`/roadmap`) and later planning (`/plan-feature`). They do not own delivery order — that lives in [TODO.md](../../TODO.md) after `/roadmap` approval. They do not own PR/commit delivery plans — those live in [active ledgers](../active/README.md) once work begins.
...
**Bullet-only MVP items in CONCEPT.md are enough to run `/roadmap`** — the command still produces a dependency-aware sequence, with lower confidence until planned specs ground the dependencies. Add specs later to firm up ordering before `/plan-feature`.
...
Delete or move the spec to the active ledger when `/plan-feature` promotes the feature to `features/active/`. Do not keep duplicate truth in both places.
