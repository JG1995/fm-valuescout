...
- Rust positive integration tests use checked-in copies of the exact pinned Youth fixture and the accepted Moneyball example, with source revisions and fingerprints recorded beside the fixtures. Small synthetic fixtures cover malformed input, alternate delimiters, quoting, BOM, and other focused boundaries. No test or build may depend on ignored `.work/` paths. Temporary SQLite databases and files cover reconciliation. Frontend tests mock the dialog and IPC boundaries; Playwright proves the bounded Dashboard flow with its existing IPC stub where practical. A real Tauri manual check owns native-dialog integration.
...
- The selected Youth fixture has SHA-256 `5c3a4a686f7619f7001e01530d4d44576718f6b3c131a7bb1a5faf94b99301a6`: 66 semicolon-delimited columns, 74 consistent rows, complete CA/PA and attribute groups, `AT Apps`, `Int Apps`, and `AT Gls`, and 74 unique positive numeric UIDs.
...
- The accepted `.work/moneyball_stats.csv` evidence has SHA-256 `2c4abe2653985206e1a1a4681dfb8f72bd11ab26906239060b5ff370bf16a8ce`: 84 semicolon-delimited columns, 75 consistent rows, 75 unique positive numeric UIDs, and representative transfer ranges, dot decimals, unit suffixes, and substitute-appearance notation.
- The pinned Youth fixture and accepted Moneyball example share 74 exact UID/name pairs; Moneyball contains one additional player. This proves cross-export identity for these two inputs, not equality with a bridge snapshot.
...
- Use the exact pinned `2030_07_01_Full_Squad_CA_PA_Monza.csv` as the Youth positive integration fixture. Do not use `.work/youth_academy_stats.csv`; the developer identified it as inaccurate.
- Promote the accepted Moneyball example into a checked-in positive integration fixture during Commit 2. Keep the ignored `.work` file as temporary evidence only.
- Map the fixture's `Int Apps` column to international appearances/caps even though the earlier Youth parser fails to recognize that real header.
...
**Work:** Add the Rust `csv_import` parser foundation, a standards-compliant CSV dependency, the pinned Youth Tracker dialect/header aliases, typed exported values, duplicate/UID validation, the exact pinned 2030 Monza positive fixture, and focused synthetic boundary fixtures/tests.
...
- Owners and files: `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/src/features/mod.rs`, new `src-tauri/src/features/csv_import/` parser/model/error files, a checked-in copy of `2030_07_01_Full_Squad_CA_PA_Monza.csv` with provenance, and small synthetic fixtures under that feature.
- Existing patterns to verify: `features/memory_read/dump_validation.rs` for fail-closed boundary errors; `features/snapshot/ingest.rs` for null preservation; the exact pinned Youth fixture, parser, and header links in Current-state map.
- Constraints and invariants: require a positive numeric UID and name header; parse all 74 rows and 66 columns of the pinned fixture; map `Int Apps` to international appearances/caps; support comma/semicolon with deterministic dialect selection; accept pinned case-insensitive aliases and appearance notation; preserve missing values; reject duplicates; and keep the module free of SQL/Tauri/UI work.
...
**Validation:** Run `./scripts/dev check`. Evidence must show the exact pinned fixture parses as Youth Tracker with 74 unique players, CA/PA and representative attribute values, and the `Int Apps` mapping. Focused tests must cover both delimiters, quoted delimiters/escaped quotes, optional UTF-8 BOM, aliases, appearance totals, blank/null values, malformed populated values, invalid/out-of-range UIDs, row-width errors, and duplicate UIDs while the full repository gate remains green.
...
- Owners and files: existing `src-tauri/src/features/csv_import/` parser/model/error files plus a checked-in copy of the accepted Moneyball example with provenance and focused Moneyball fixtures/tests.
- Existing patterns to verify: pinned Moneyball `ingest.rs`, `row.rs`, `parsers.rs`, and exercised fixture headers; `.work/moneyball_stats.csv` only as the fingerprinted source for the checked-in positive fixture; commit 1's shared CSV/error/UID contracts.
- Constraints and invariants: parse all 75 rows and 84 columns of the accepted example; preserve its semicolon dialect, exact current long-form headers, optional Division/CA/PA/Save Percentage groups, transfer ranges, dot decimals, units, and appearance notation; support the pinned alias policy and first-header BOM; require numeric UID; preserve nulls; reject duplicates; and parse only physically exported values.
...
**Validation:** Run `./scripts/dev check`. Evidence must show the accepted 84-column example parses as Moneyball with 75 unique players and representative transfer, height, distance, appearance, percentage, and statistic values. Focused tests must cover required/optional headers, old/new aliases, semicolon and BOM behavior, nulls, malformed identity/data, duplicates, format auto-detection, and explicit rejection of a near-match missing a required Moneyball group.
...
**Validation:** Run `./scripts/dev check`. Evidence must show Rust tests for Youth and Moneyball summaries, including a snapshot seeded from the 74 pinned Youth UIDs where the accepted Moneyball fixture reports 74 matches and one unmatched row. Tests must also cover general exact matches/unmatched counts, no snapshot, stale save/snapshot, duplicate/invalid data propagation, file type/size/UTF-8 limits, safe bounded errors, no full-row IPC response, and unchanged migration version/table contents after success and failure.
