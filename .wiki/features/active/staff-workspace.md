...
- Feature-complete review round 1 retained bridge index continuity, Staff cache invalidation, Staff sort parity, truthful partial-progress errors, internal job-ID labeling, money formatting, and current-state documentation gaps. The correction pass addresses those contracts before the same reviewer rechecks the feature.
...
| PR 2 | Color Staff Profile attributes | `427c7ac` | Staff and Player Profiles share one attribute row and value presentation with identical spacing, typography, borders, FM-scale tier colors and labels, and neutral missing values | Sol Medium accepted after one correction round strengthened shared row ownership and visual regression proof | None |
| PR 2 | Decode Working With Youngsters as a raw attribute | `ad70c90` | The bridge reads the 1–20 value directly instead of applying the reputation conversion used by the adjacent field | Focused bridge regression and full bridge suite passed | Fresh supported-build Load Data was explicitly waived by the developer |
...
8. Inspect Staff at 1280×800 and 1600×900 in the native Tauri shell, including horizontal scrolling, column menus, reorder/resize, filter editing, table-to-profile navigation/back state, the three-column profile attribute panel and bounded Role fit panel, revealed/concealed states, empty/error/setup states, keyboard navigation, modal focus, announcements, and loading stability.
