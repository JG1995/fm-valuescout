...
**Delivery fingerprint:** 8cb75ca05c99424144dd6662ea12953b27b482a00e5b3cd96df3a7acce96c4ea
...
[ADR-0023](../../decisions/0023-lazy-club-dna-score-cache.md) remains the accepted cache decision. This correction changes only the indexed read shape within that decision, so it needs no migration, index, ADR decision, current-state architecture, design, TODO, or BACKLOG change. Its implementation-status sentence drops the stale commit ordinal.
...
- Commits 1–5 are complete at `ddd4961e6d90ca24faa435955c6ae7eb5a716f0b`, `d2682ee5c50cb99cd0b7f9facf5fd4f9060d5001`, `7cf5e5924af8a9c54852f5037e17ffe4b2c58cc0`, `d78f97f25497409f6c895a8ac5cdeb74ea5301eb`, and `df074b9da78bec038960e2be7c851dcb5879dbdd`. The failed Commit 6 worktree was discarded cleanly.
- Committed v32 provides definition versions, nullable cache rows, exact cache identity, the score lookup/order index, bounded lazy materialization, and invalidation. Search and Squad do not expose Club DNA at clean HEAD `df074b9da78bec038960e2be7c851dcb5879dbdd`.
- The failed indexed 2,000-player run reported all shapes. Search filter cold first use was 25.054 ms with warm p95 10.655 ms. Search sort cold first use was 26.239 ms with warm p95 12.295 ms. Squad sort cold first use was 28.243 ms with warm p95 15.275 ms.
- The failed indexed 184,000-player run reported all shapes. Search filter cold first use was 2845.481 ms with warm p95 1116.122 ms. Search sort cold first use was 2953.205 ms with warm p95 1207.320 ms. Squad sort cold first use was 3411.322 ms with warm p95 1703.075 ms. All three warm shapes breached the `<=200 ms` representative threshold.
- The confirmed root cause is warm global-request completeness detection. Each warm request still enumerated every cohort UID and then ran about 736 per-250 `NOT EXISTS` cache-miss probes although the exact-version cache was already complete.
- Diagnostic medians, which are not acceptance measurements, were: snapshot UID list 42.8 ms; batched miss probes 109.6 ms; exact snapshot cache count 4.9 ms; snapshot player count 4.4 ms; Search filter count 44.5 ms; Search sort page 100.5 ms; Squad membership count 40.6 ms; exact Squad cache count 64.5 ms; and Squad sort page 243.8 ms.
...
- Do not add a durable completeness marker, migration, index, ADR, background job, progress/cancellation design, or frontend behavior change for this correction.
...
- Completeness-fast-path timings are not yet measured. Commit 7 must run the three exact deterministic shapes in the 2,000-player benchmark test and the three shapes in the generated 184,000-player benchmark test, with cold results separate from warm samples and each test asserting its own size threshold.
...
Committed v31/v32 persistence and materialization remain the foundation. Commit 6 records this bounded completeness correction. Commit 7 adds one request-scoped trusted definition/model context, page-first display with one exact-version `LEFT JOIN`, cache-first global Search and Squad operations driven by the v32 score index, and the exact-count warm-cache fast path before UID enumeration. Commits 8–10 then deliver the unchanged frontend metric, Modal, and My Club integration.
...
#### Commit 7 — Integrate indexed queries with the completeness fast path
...
- Freshly implement the complete corrected indexed-query integration from clean HEAD `df074b9da78bec038960e2be7c851dcb5879dbdd`. Replace every Club DNA correlated scalar read with one request-scoped trusted definition/model context and fixed cache aliases. Keep display player-first and page-scoped. Before any global UID enumeration or materializer call, compare exact-version cache-row count with the authoritative operation cohort count. Skip all UID allocation, materializer calls, and batched miss probes on equality; otherwise use the existing resumable materializer. After completeness, drive global Search filter/sort and Squad sort from exact-version `club_dna_scores` rows so the committed v32 score index owns count, order, and page selection.
...
- `src-tauri/src/features/search/query.rs` — fetch one trusted context after request validation; reuse it for materialization and SQL binds; keep display player-first, materialize only selected page UIDs, and `LEFT JOIN` one exact-version cache alias in the final select; for Club DNA filter or sort, compare the exact-version snapshot cache count with the snapshot player count before UID enumeration, skip all materializer work on equality, otherwise resume existing materialization, then use cache-first count/order/page SQL driven by the v32 score index before the bounded player select; add correctness, completeness, stale/missing-definition, page-scope, EXPLAIN, and benchmark-runner proof.
- `src-tauri/src/features/planner/squad.rs` — fetch one trusted context after request and configured-cohort validation; keep display page-first with one exact-version `LEFT JOIN`; for Club DNA sort, obtain the exact managed-club cohort count, compare it with exact-version cache rows joined to that same managed-club predicate before UID enumeration, skip all materializer work on equality, otherwise resume existing exact-cohort materialization, then drive order/page from exact-version cache rows and join only cohort members; reuse the cohort count only where the exact same validated predicate makes it safe.
...
9. Add `EXPLAIN QUERY PLAN` assertions for Search filter, Search sort, and Squad sort. Require score-index selection on the committed v32 `(snapshot_id, definition_version, score_model_version, score)` index and reject correlated or player-first global plans. A temporary sort is allowed only for the explicit UID tie-break after indexed score selection.
...
- RED: clean HEAD has no Club DNA resolver/filter query contract or completeness fast path. The discarded failed Commit 6 proof reported all indexed shapes but breached every 184,000-player warm threshold because each request still enumerated the full cohort and ran about 736 per-250 miss probes against an already complete cache. A test that permits player-first correlated SQL, complete-cache UID enumeration, any complete-cache materializer/miss-probe call, or early benchmark exit is insufficient.
...
- Performance proof: run correctness assertions before accepting any timing. Within each size-specific test, clear the cache for each exact shape, measure cold first use separately, prefill, run 3 unmeasured warm-ups and 20 measured samples, and compute nearest-rank p95 from sorted index 18. The 2,000-player test executes and reports its three shapes before asserting every p95 `<500 ms`. The generated 184,000-player test executes and reports its three shapes before asserting every p95 `<=200 ms`. The two commands cover six combinations without any cross-process aggregate. The prior Squad sort-page diagnostic median of 243.8 ms is not acceptance evidence; it is an explicit review focus.
- Add/modify: request-context and materializer instrumentation tests in `player_metrics/club_dna.rs`; resolver/filter tests; Search completeness, correctness, EXPLAIN, fixture, and benchmark helpers in `search/query.rs`; Squad completeness, cohort-transition, correctness, and query-plan coverage in `planner/squad_tests.rs`. Deliberately retain migration/index characterization, pure scoring, materializer batching/rollback, raw attribute, role, potential-role, current-snapshot, managed-club, pagination, null-ordering, and request-bound tests because they protect independent current contracts. Delete only failed Commit 6 worktree helpers if any reappear; none exist at clean starting HEAD. No mocks or snapshots change.
...
- Keep ADR-0023, v32 schema/index, cache identity, lazy scopes, invalidation, versions, formula, mutex/transaction boundaries, UX, thresholds, and frontend behavior unchanged. Do not add a durable completeness marker.
...
- Depends on completed Commit 4's v32 cache/index, pure scorer, bounded materializer, and invalidation; completed Commit 5's reviewed indexed architecture at `df074b9da78bec038960e2be7c851dcb5879dbdd`; and reviewed Commit 6's bounded completeness correction. Commit 8 may expose the fixed metric only after Commit 7 passes completeness, correctness, EXPLAIN, 2,000-player, and generated 184,000-player warm gates.
...
- Depends on Commit 7 so every exposed metric request is already accepted, completeness-checked, and cache-backed in Rust. The later Modal and route packets consume these adapters and shared groups.
...
#### Commit 9 — Build the Club DNA definition Modal
...
- Depends on Commit 8's typed API and shared catalog. Commit 10 provides route placement and cross-query/layout effects.
...
#### Commit 10 — Integrate Club DNA with My Club and layouts
...
- Depends on Commits 2–9. It is the final implementation packet and moves the feature to Validation only after its full checkpoint clears.
...
**Commit:** Commit 7 — Integrate indexed queries with the completeness fast path

### RED or removal proof

Add focused `club_dna_completion_fast_path_` tests that fail while complete caches still enumerate cohort UIDs or run batch miss probes. Add request-context, resolver, filter, Search, Squad, no-definition, query-plan, cohort-transition, and exact benchmark proofs from the unchanged packet.

### Expected outcome

Exact cohort-count completeness eliminates warm materializer overhead while indexed cache-first global SQL and page-first display preserve all Club DNA behavior and both performance gates.

### Explicit exclusions

Migrations, indexes, durable completion markers, ADR decisions, definition/cache lifecycle changes, frontend code, eager ingest or promotion, Moneyball, Player Profile, background work, progress/cancellation, and current-state documentation.
...
- Commit 5 recorded the indexed query-shape correction at `df074b9da78bec038960e2be7c851dcb5879dbdd`. The later failed Commit 6 worktree was discarded, so replacement implementation starts fresh from that clean HEAD.
- The failed indexed 2,000-player run reported every shape: Search filter cold 25.054 ms and warm p95 10.655 ms; Search sort cold 26.239 ms and warm p95 12.295 ms; Squad sort cold 28.243 ms and warm p95 15.275 ms.
- The failed indexed 184,000-player run also reported every shape: Search filter cold 2845.481 ms and warm p95 1116.122 ms; Search sort cold 2953.205 ms and warm p95 1207.320 ms; Squad sort cold 3411.322 ms and warm p95 1703.075 ms. Every warm representative shape breached `<=200 ms`.
- Investigation confirmed that every warm global request still enumerated all cohort UIDs and ran about 736 per-250 `NOT EXISTS` miss probes after the cache was complete. Diagnostic medians were snapshot UID list 42.8 ms, batch probes 109.6 ms, exact cache count 4.9 ms, snapshot count 4.4 ms, filter count 44.5 ms, sort page 100.5 ms, Squad membership count 40.6 ms, exact Squad cache count 64.5 ms, and Squad sort page 243.8 ms. These medians guide review but are not acceptance evidence.
- The bounded correction keeps v32, ADR-0023, indexed query architecture, UX, scopes, versions, thresholds, formula, invalidation, frontend behavior, product behavior, and one-PR authority unchanged. It adds exact-version cohort-count completeness before UID enumeration. Search compares exact-version cache rows with snapshot players. Squad counts only exact-version rows joined to the same exact managed-club predicate. Nullable rows count; outside-club rows do not; primary and foreign keys prevent duplicate or orphan false matches.
- Count equality skips UID allocation, every materializer call, and every batch miss probe. Count mismatch uses the existing resumable materializer. Additions and club entries without an exact current-version row mismatch. Player removals cascade both Search counts. Club exits drop from both Squad predicates. Entrants already materialized by Search and switches to already complete clubs can preserve equality and must skip safely. The implementation may reuse a cohort count only for the same validated snapshot and predicate. No durable marker, migration, index, or ADR decision change is needed.
- This packet change invalidated Delivery fingerprint `4917d5fd65279b9390c2fac5fd37448561996367b7e4a41c129a1868a16cc03a`. Independent review cleared the completeness correction, the developer accepted it and re-invoked delivery, and the ledger records replacement fingerprint `8cb75ca05c99424144dd6662ea12953b27b482a00e5b3cd96df3a7acce96c4ea`.
- Each size-specific benchmark test executes and reports its own Search filter, Search sort, and Squad sort shapes before asserting its own unchanged threshold. The normal 2,000-player and ignored generated 184,000-player commands together cover six combinations, but no aggregate assertion crosses their process boundary. Both retain separate cold measurements, 3 warm-ups, 20 measured samples, and nearest-rank p95. Any warm breach stops delivery. The 243.8 ms Squad sort-page diagnostic median is not acceptance, but it is an explicit review focus.
- The frontend attribute catalogs remain duplicated between Player Profile grouping and player metric metadata. Commit 8 still consolidates them into a shared utility; Rust retains its independent authoritative catalog.
- `.wiki/TODO.md`, `.wiki/BACKLOG.md`, `.wiki/ARCHITECTURE.md`, and `.wiki/DESIGN.md` remain unchanged during this bounded replan. ADR-0023 changes only its implementation-status sentence; its decision and thresholds remain unchanged.
