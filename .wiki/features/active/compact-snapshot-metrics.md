...
**Commit:** Commit 3 — Preserve visible data until successful replacement

### RED or removal proof

Hook and composition tests fail because Load Data still uses the neutral context-mutation key and clears Search/Squad roots before invoking Tauri, which unmounts or blocks established results during long work and loses them on failure.

### Expected outcome

Load Data captures the active save context, keeps existing Search and Squad results mounted and interactive during progress, ignores stale work after a save switch, and clears exact roots only after a context-matching successful effective-current replacement; historical non-winners and failures preserve current results; focused frontend tests, `./scripts/dev check-app`, and `./scripts/dev check` pass.
