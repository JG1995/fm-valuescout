...
**Delivery fingerprint:** 88e336f2a14056315cb4e250fe0d9a97cb9ca882483d4d87c3674e9bf968eaf6
...
- Relevant components:
  ...
  - `src/features/planner/api/planner-keys.ts` + `src-tauri/src/features/planner/tactic.rs` + `commands.rs` — the pre-Commit-6 implementation resolves the active save implicitly. Commit 6 (IPC seam) changes the existing read/options/save IPC signatures to captured `{ saveId, contextToken }`, validates save existence and token identity without `is_active`, and keys frontend queries/mutations by the full context. Commits 7 and 8 add My Club lifecycle/errors and Search sort/no-snapshot respectively; Commit 9 adds toggles/interleaving/header-X. Transaction-scoped tactic helpers keep validation and each read/default insert/save in one command-owned transaction.
...
- Project validation commands:
  ...
  - Branch reality: feature branch `feat/tactic-table-columns` is active at `HEAD f8d14d5` (`docs(tactic-table-columns): simplify save context plan`) — the clean IMPLEMENTATION baseline before this current ledger-only planning edit; current worktree is not globally clean: only `.wiki/features/active/tactic-table-columns.md` is modified; incomplete implementation draft remains preserved outside worktree. Base `main` at `406dca7` is synchronized with `origin/main`. `src/lib/with-abort.ts` is absent.
- Primary risks:
...
**Depends on:** `main` at `406dca7` (synchronized with `origin/main`) plus completed commits 1–5 on `feat/tactic-table-columns`; Commit 5 is `d9d2737` and current `HEAD f8d14d5` is the clean IMPLEMENTATION baseline before this current ledger-only planning edit; current worktree is not globally clean: only `.wiki/features/active/tactic-table-columns.md` is modified; incomplete implementation draft remains preserved outside worktree.
...
**Work:** Uses the C6 remount unchanged and only adds `retryBoth`, `readOnly`, actionable initial/refresh error lifecycle, and mounted-B feedback isolation proof. It may test that existing remount still isolates draft/feedback while exercising the additive lifecycle, but does not reimplement or claim ownership of remount/key/mutation. Provide actionable initial `Could not load tactic` with Retry-both for tactic-only or options-only failures and cached refetch that retains data read-only; verify mounted-B feedback isolation while reusing C6 remount/key/mutation unchanged; same-ID/new-token transition is verified without reimplementing context wiring. No Search changes.

**Size assessment:** ~140 non-test lines + ~80 tests. Within soft target; focused on boundary+route lifecycle without Search/IPC changes.

**Out of scope:**

- Search tactic sort gating and no-snapshot composition (Commit 8).
- Rust IPC or scoring changes; toggle buttons, synthetic rendering, interleaving, header-X, widths (Commit 9).

**Implementation packet:**

**Files and responsibilities:**

- `src/features/planner/components/tactic-context-boundary.tsx` — harden the existing minimal boundary created in Commit 6 (do not recreate queries, keys, or factories). Add the additive lifecycle states only: `readOnly: boolean` and `retryBoth: () => void`. Initial failure (either query `isError` with no cached data, tactic-only or options-only) -> `initialError` renders `Could not load tactic` with `retryBoth` refetching both context-keyed queries; cached `isRefetchError` with data -> `readOnly: true` retains `tactic`/`options` and exposes `refreshError` + `retryBoth`. The two unconditional non-null context queries, keys, and factories remain owned by Commit 6.
- `src/features/planner/components/planner-tactic-editor.tsx` — no new key/factory/mutation wiring; Commit 6 already owns full-context `mutationKey`/`variables`/cache write and full-context remount. This commit only verifies additive feedback/draft lifecycle beyond basic remount (delayed A completion's feedback isolation in mounted B).
- `src/app/routes/my-club.tsx` — consume the hardened boundary's `readOnly`/`retryBoth`/`refreshError` to show actionable `Could not load tactic` with Retry-both on initial failure, retain cached data read-only on refetch failure, block writes while `readOnly`, and surface route/editor feedback on context switch beyond basic remount. No new `TacticContextBoundary` creation, query factory, mutation key, or remount wiring; that was Commit 6.

**Behavior and data flow:**

- Initial tactic-only or options-only error shows `Could not load tactic` with Retry-both that refetches both context-keyed queries (hardened boundary).
- Cached refresh error retains tactic/options and labels read-only until Retry succeeds; writes are blocked via `readOnly` while `refreshError` is shown.
- Beyond Commit 6's basic full-context remount (which already isolates draft/feedback via keyed unmount), this commit verifies route/editor feedback/draft lifecycle on context switch (clearing stale feedback, preserving readOnly blocking) and delayed A completion's mounted-B feedback isolation: A may populate `plannerKeys.tactic(contextA)` but B's mounted editor shows no A draft/feedback and B's cache/labels remain unchanged.

**Ordered implementation steps:**

1. Harden the existing `TacticContextBoundary` (from Commit 6) to expose `readOnly` and `retryBoth`; ensure `retryBoth` refetches both context-keyed queries and `readOnly` is derived from cached `isRefetchError`.
2. Update `my-club.tsx` to consume `readOnly`/`retryBoth`/`refreshError` for actionable initial and cached errors; preserve existing snapshot/saves/Define DNA behavior. Do not touch query/mutation keys or editor remount wiring.
3. Add focused lifecycle/error tests covering Retry-both (tactic-only and options-only), cached read-only blocking, route/editor feedback on context switch beyond basic remount, and delayed A completion's mounted-B feedback isolation; run validation and obtain fresh review.

**Tests and proof:**

- Boundary additive tests: tactic-only and options-only initial errors show `Could not load tactic` and Retry-both refetches both keys; cached refetch failure retains data read-only and blocks writes until recovery.
- Lifecycle additive tests: route/editor feedback/draft lifecycle on context switch beyond basic remount (stale feedback cleared, readOnly blocking); delayed A completion may finish for A (A cache/persisted value updated) but mounted B shows no A feedback/draft and B's cache/labels remain unchanged (feedback isolation).
- Same-ID/new-token transition: old-token operation cannot affect new save's editor/cache; new-token mount fetches new context.
- Fixed-string: no new `plannerKeys.tactic` factory, no new `mutationKey` wiring, no parent `currentContextRef` tactic settlement guard, no `withAbortSignal`, no global pending helpers.

**Patterns to verify:**

- Harden existing child boundary (unconditional queries owned by Commit 6) — do not recreate queries or factories.
- Additive `readOnly`/`retryBoth` derive from existing queries; no new mutation wiring.

**Constraints and non-goals:**

- Do not add cancellation, active-only validation, sentinels, nullable factories, conditional hooks, global pending helpers.
- No Search file changes; do not change Rust or IPC.

**Dependencies and sequencing:**

- Depends on Commit 6 (captured-context IPC seam). Search Commit 8 is independent of this commit's UX but shares Commit 6.

**Validation:** `./scripts/dev check-app`, `./scripts/dev test src/app/routes/my-club-squad.test.tsx`, `git diff --check`, LSP diagnostics, fixed-string proofs (`withAbortSignal`, `with-abort`, `pendingPlannerTacticDeferred` absent). Smoke optional after check.

**Stop conditions:** Stop if boundary requires nullable factories or conditional hooks to be hook-safe, or if lifecycle requires parent refs/stale guards.

**Review mandate:**

1. Initial tactic/options-only errors are actionable Retry-both; cached refresh retains data read-only (additive UX, not rewiring).
2. Route/editor feedback/draft lifecycle on context switch beyond basic remount is correct; delayed A completion's mounted-B feedback remains isolated.
3. No duplicated `plannerKeys.tactic`/`plannerKeys.tacticOptions` factories, no duplicated `mutationKey`/`variables` wiring, no reimplemented full-context remount; those remain Commit 6.
4. Same-ID/new-token transition distinct and safe.
5. No Search or Rust changes.

**Acceptance criteria:**

1. My Club shows Retry-both on initial tactic/options failure, read-only retained data on cached refetch failure.
2. Context switch via the existing C6 remount still isolates draft/feedback — C7 verifies that remount while exercising additive `readOnly`/`retryBoth` lifecycle, does not reimplement remount/key/mutation; delayed A completion's mounted-B feedback remains isolated.
3. Same numeric ID with new token uses distinct cache/editor state.
4. No Search IPC or backend changes.

#### Commit 8 — Search tactic sort gating and no-snapshot composition

**Status:** Pending

**Provisional commit:** `feat(search): add tactic sort gating and no-snapshot composition`

**Work:** Add layout-aware tactic URL sort acceptance/fallback plus retain tabs/filter strip for null/mismatched snapshot with zero Search tactic IPC. The tactic query boundary remains deferred to Commit 9 (UI packet).

**Size assessment:** ~90 non-test lines + ~60 tests. Within soft target; focused route + filter-strip composition.

**Out of scope:**

- My Club lifecycle/error changes (Commit 7) and backend/Rust changes.
- Search toggle buttons, labels, synthetic rendering, interleaving, header-X recompaction, widths, null-last polish (Commit 9); no Search `TacticContextBoundary` mount.

**Implementation packet:**

**Files and responsibilities:**

- `src/features/search/utils/dynamic-columns.ts` — `isVisibleSortField` accepts a canonical `tactic_current.*`/`tactic_potential.*` sort only when that exact ID is present in current view's persisted `layouts[tableId].columnIds` (synthetic allowlist, not `getFilterField`). Otherwise reject.
- `src/app/routes/search.tsx` — `validateSearch` mirrors same allowlist; invalid/removed/wrong-table tactic sort falls back to `defaultSearchSort(view)`. Keep `SearchFilterBar` (tabs + filter strip) mounted when active immutable save context `{ saveId, contextToken }` exists but `snapshot == null` or `snapshot.saveId !== context.saveId` (`isMatchedSnapshot` false); only `SearchResultsPanel` shows empty/loading. Do not mount `TacticContextBoundary` or fire `get_planner_tactic`/`get_planner_tactic_options`/`save_planner_tactic` in this commit; no toggle handler, click, message, or non-ready toggle behavior in this commit (full toggle UI and `No snapshot loaded — use Load Data` proof are Commit 9).

**Behavior and data flow:**

- `search?view=general&sort=tactic_current.goalkeeper` restores when that exact ID is in `layouts[search].columnIds`; removing the column or switching view makes sort fallback to default. Header sort click is later (Commit 9); URL validation is proven here.
- No-snapshot: `context != null && !isMatchedSnapshot` -> tabs + `SearchFilterBar` remain mounted; results panel empty; zero tactic IPC because boundary not mounted; layout unchanged.

**Ordered implementation steps:**

1. Extend `isVisibleSortField` + `validateSearch` with persisted-layout allowlist for tactic sort IDs.
2. Refactor `search.tsx` to keep tabs/filter strip mounted for null/mismatched snapshot; gate tactic IPC to matched snapshot only (boundary still not mounted).
3. Add focused route tests; run validation; review.

**Tests and proof:**

- Sort-gating tests: valid present tactic sort restores; absent/invalid/wrong-table tactic sort falls back to `defaultSearchSort(view)`; header-X removal later falls back but URL fallback proven here.
- No-snapshot tests: `snapshot == null` or `snapshot.saveId !== context.saveId` preserves tabs/filter controls, issues zero `get_planner_tactic`/`get_planner_tactic_options`/`save_planner_tactic`, leaves layout unchanged (empty/loading result panel, unchanged layout); no toggle handler or message asserted here (that proof is Commit 9).
- Fixed-string: no `get_planner_tactic` IPC in Search path for no-snapshot; boundary mount gated by `isMatchedSnapshot`.

**Patterns to verify:**

- Existing `isVisibleSortField` delegation to `getFilterField` for non-tactic IDs preserved; tactic branch first checks `isValidTacticColumnId` + layout presence.
- `search.tsx` filter strip mounting pattern (previously early return) now branches only panel.

**Constraints and non-goals:**

- Do not mount Search tactic boundary, add My Club/backend changes, or claim interleaving/header-X/toggle UI.
- Keep simplified semantics; no stale-result discard helpers.

**Dependencies and sequencing:**

- Depends on Commit 6 (IPC seam) and Commit 3 (atomic layouts) and Commit 5 (scorer) for sort correctness, but not on Commit 7. Commit 9 depends on this commit.

**Validation:** `./scripts/dev check-app`, `./scripts/dev test src/features/search/utils/dynamic-columns.test.ts src/app/routes/search.test.tsx`, `git diff --check`, LSP diagnostics.

**Stop conditions:** Stop if sort validation must query Rust or if tabs/filter strip cannot be retained without mounting tactic IPC.

**Review mandate:**

1. Tactic sort visible only when exact ID in current table layout; otherwise fallback.
2. Tabs/filter strip retained for null/mismatched snapshot; only panel shows empty.
3. Zero Search tactic IPC when no matched snapshot; layout unchanged.
4. No My Club/backend changes.

**Acceptance criteria:**

1. Valid tactic URL sort accepted; invalid/removed/wrong-table falls back.
2. No-snapshot composition retains tabs/filter strip, shows empty panel, fires zero tactic IPC, and does not mutate layout.
3. Boundary still deferred to Commit 9; no Search tactic fetch yet.

#### Commit 9 — Toggle buttons, interleaving, header wiring, widths and null-last polish
...
- Additional backend logic beyond what commit 5 provides; Store atomic action definition beyond what commit 3 provides; route sort validation beyond what commit 8 provides; metric picker promotion.
...
- `src/features/search/utils/dynamic-columns.ts` / `src-tauri/src/features/search/query.rs` — no further change in this commit beyond width/sort polish verification (Moneyball mapping doc comment lives in commit 5 per NITPICK; do not duplicate in commit 7 or 8).
...
- Depends on commits 2–8 (helpers + store atomic action + resolver + query scorer + captured-context `{ saveId, contextToken }` IPC seam in Commit 6 plus My Club lifecycle in Commit 7 and Search sort/no-snapshot in Commit 8). Commit 9 reuses `plannerKeys.tactic(context)`/`plannerTacticQueryOptions(context)` (context-keyed, numeric ID reuse isolated) from commit 6 plus the atomic `replaceLayout` from commit 3 for header-X recompaction and interleaving; no new IPC; ensure `src/lib/with-abort.ts` remains deleted (fixed-string `rg -F "with-abort"` zero).
...
- Route/sort boundary discovered: `isVisibleSortField` gated non-basic sorts on `getFilterField`, so synthetic tactic IDs cannot pass. Current plan assigns the layout-aware sort acceptance change to Commit 8 landing before the toggle UI (Commit 9), ensuring every exposed sort is already trunk-safe and tested for header, URL restoration, and fallback after removal. Historical note: prior draft assigned this to commit 6/7 before the split — superseded.
- Store atomicity discovered: `removeColumn`/`addColumns` only drop one ID/width each, leaving interleaved partials after header-X. Correction defines one atomic `replaceLayout` action in commit 3 validated and used by both toggle and tactic-aware header-X paths in Commit 9 (via `replaceLayout`) with deterministic post-X straight recompaction. Historical note: prior draft referenced commit 6/7 for this path — superseded by the C6→C7→C8→C9 split.
- Save-scoping and no-snapshot discovered: `plannerTacticQueryOptions` is save-independent while `get_planner_tactic` resolves `active_save_id` implicitly, and `search.tsx` early-returns before the filter strip when `!snapshot`, contradicting the toggle-remaining promise. Current plan makes the query explicitly immutable-context-scoped (`plannerKeys.tactic({ saveId, contextToken })` analogue `clubDnaKeys.definition` at `src/features/club-dna/api/club-dna-keys.ts:5`, `fetchPlannerTactic({ saveId, contextToken })` with exact camelCase `{ saveId, contextToken }`, Rust `get_planner_tactic(save_id, context_token)`/`get_planner_tactic_options(save_id, context_token)` validating via reusable `pub(crate) ensure_save_context` at `src-tauri/src/features/snapshot/service.rs` with two-step distinct `Save {id} not found` vs `Save changed or no longer exists` inside the command-held `Db` lock + `Transaction` before read/default init, with `get_tactic` default initialization using single `tx` via `save_tactic_in_tx`/`load_tactic_from_tx` without nesting, and numeric ID reuse isolated via immutable `{ saveId, contextToken }` query keys). Responsibility split: C6 owns IPC and the minimal My Club boundary (transaction helpers, captured-context commands, TS adapters/keys/factories/mocks, minimal `TacticContextBoundary` with its two unconditional non-null context queries/minimal states, context mutation/key/cache wiring, and full-context remount with basic proof); C7 owns My Club lifecycle hardening only (adds `retryBoth`, `readOnly`, actionable initial/refresh error lifecycle, and mounted-B feedback isolation proof reusing C6 remount unchanged); C8 explicitly owns Search filter-strip/no-snapshot/sort behavior (`isVisibleSortField`/`validateSearch` layout-aware allowlist and `search.tsx` retaining tabs + `SearchFilterBar` for null/mismatched snapshot with zero Search tactic IPC — only `SearchResultsPanel` branches, no toggle/message in C8); C9 owns Search boundary consumption, toggles, Retry/read-only UI, labels/rendering/interactions (`TacticContextBoundary` consumption in Search, `TacticColumnToggles`, `laneLabels` from `PlannerTactic` + `TacticOptions`, synthetic cells/interleaving/header-X/widths/null-last). Prior assignment of Search sort/no-snapshot to Commit 6 and Search toggles/labels to Commit 7 is explicitly superseded — superseded by the C6→C7→C8→C9 split. Component-owned non-throwing fetch preserves initial `isError` -> `Retry` and cached `isRefetchError` retains data read-only, and preserves existing `currentSnapshotQueryOptions`/`savesQueryOptions` loader propagation and Define DNA `clubDnaQueryOptions(context)` behavior (no `catch(()=>null)` swallowing). Historical note: prior drafts described this as `useQuery({ ...plannerTacticQueryOptions(context), enabled: context != null && isMatched })` with nullable factories — superseded by conditional boundary mount + unconditional child queries in C6. Consumers/mocks `src/testing/setup.ts` + `e2e/tauri-ipc-stub.ts` and test-local immutable context-keyed delayed A/B via `contextKey({ saveId, contextToken })` (analogue `src/features/club-dna/components/club-dna-definition.tsx:67`) are wired via C6's context-keyed IPC seam.
...
- Commit 6 simplification after the second failed implementation cycle: the developer judged the human race window for a local tactic save too small to justify active-only rejection and complete stale-settlement suppression. The accepted replacement contract captures `{ saveId, contextToken }` when an operation starts, targets that exact save even if it becomes inactive, validates row existence plus token identity, and stores results only under the originating context key. This removes `is_active` from tactic context validation, parent-owned mutation settlement guards, discard-only cache assertions, and one-active-context mocks. Commit 9 keeps the same UI state model and context-keyed reads; its only test change is that delayed A may populate A's cache while B's labels/cache remain unchanged.
- Developer-approved correction (2026-09-03, HEAD `f8d14d5`): prior plan confused atomicity with bundling adjacent concerns in old Commit 6 (IPC seam + My Club lifecycle + Search sort/no-snapshot). The third monolithic implementation attempt was stopped before another correction cycle; splitting prevents repeat failure while preserving exact feature scope. Old Commit 6 is replaced by three trunk-safe commits — 6 IPC seam, 7 My Club lifecycle/errors, 8 Search sort gating/no-snapshot — and old Commit 7 is renumbered to Commit 9. No new IPC, no temporary implicit-active compatibility API, no parallel command; each packet contains complete Work/size/out-of-scope/implementation packet/files/behavior/steps/tests/patterns/constraints/stop/review/dependencies/validation/acceptance. Branch is `feat/tactic-table-columns` at `HEAD f8d14d5` — the clean IMPLEMENTATION baseline before this current ledger-only planning edit; current worktree is not globally clean: only `.wiki/features/active/tactic-table-columns.md` is modified; incomplete implementation draft remains preserved outside worktree.
...
- No-snapshot/save-switch proof: active immutable save context present + snapshot null/mismatch (`saves=[{id:1, contextToken:"tokenA", isActive:true}]` valid, `snapshot==null` or `snapshot.saveId !== context.saveId`) -> `TacticContextBoundary` not mounted, zero `get_planner_tactic`/`get_planner_tactic_options`/`save_planner_tactic` IPC, filter strip + tabs remain present; gated toggle click proves `No snapshot loaded — use Load Data` exclusively in Commit 9 (not Commit 8) without mutating layout and without IPC (tabs/filter retained, empty/loading panel, unchanged layout, zero tactic IPC is the Commit 8 proof); switching immutable context `{ saveId, contextToken }` (`contextA {1,"tokenA"} -> contextB {2,"tokenB"}` and `contextA {1,"tokenA"} -> {1,"tokenA2"}` numeric ID reuse) changes `plannerKeys.tactic(context)` / `tacticOptions(context)` (analogue `clubDnaKeys.definition`) with distinct keys and does not leak prior save's tactic; `laneLabels` update for new context (save role selection + shared `TacticOptions` catalog `displayName`) while table-scoped `columnIds`/`widths` are preserved across save switches (commit 9 asserts) via fixed-string `rg -F "contextToken"` checks and `queryClient.getQueryData(plannerKeys.tactic(context))` assertions. Sort gating URL proof is Commit 8; toggles/labels/synthetic rendering/interleaving/header-X/widths are Commit 9; context IPC is Commit 6; My Club lifecycle is Commit 7.
