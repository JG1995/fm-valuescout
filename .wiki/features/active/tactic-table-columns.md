...
**Delivery fingerprint:** f0dfabde36a945686eb8854c662ed238ccca1e342c95a4f39501a7331c383eb9
...
- Relevant components:
  ...
  - `src/features/planner/api/planner-keys.ts` + `src-tauri/src/features/planner/tactic.rs` + `commands.rs` — the pre-Commit-6 implementation resolves the active save implicitly. Commit 6 changes the existing read/options/save IPC signatures to captured `{ saveId, contextToken }`, validates save existence and token identity without `is_active`, and keys frontend queries/mutations by the full context. Transaction-scoped tactic helpers keep validation and each read/default insert/save in one command-owned transaction.
...
- Project validation commands:
  ...
  - Branch reality: feature branch `feat/tactic-table-columns` is active at `HEAD 86461ac` (the accepted replan documentation commit), with completed Commit 5 at ancestor `d9d2737`. Base `main` at `406dca7` is synchronized with `origin/main`. The failed Commit 6 implementation remains uncommitted across 20 changed or untracked paths; `src/lib/with-abort.ts` is absent. Historical note: planning-time work was staged on `release/v0.15.0` with `main` behind `origin/main`.
- Primary risks:
...
**Depends on:** `main` at `406dca7` (synchronized with `origin/main`) plus completed commits 1–5 on `feat/tactic-table-columns`; Commit 5 is `d9d2737` and current `HEAD 86461ac` adds the accepted replan record. Failed Commit 6 work remains uncommitted.
...
#### Commit 6 — Captured save-context Planner tactic IPC, route sort gating, and no-snapshot composition

**Status:** Active

**Provisional commit:** `feat(search): target tactic IPC to captured save context`

**Work:** Replace the implicit-active-save Planner tactic reads, options read, and save mutation with explicit captured context `{ saveId, contextToken }`. Each Rust command operates on that exact save context even if the user activates another save before the command runs. The command rejects an unknown save ID or a mismatched context token, but it does not reject a valid context only because the save became inactive. This makes a fast tactic save safe without cancellation or complete stale-completion suppression: an operation started for A may finish for A, and it must never read or write B. Keep query and mutation keys context-scoped so an A result can populate only A's cache. Also add the existing error/retry boundary, layout-aware tactic sort validation, and Search no-snapshot composition. Commit 7 still owns tactic toggles, labels, synthetic rendering, interleaving, header-X behavior, and widths.

**Size assessment:** ~210 non-test lines + ~120 tests. The IPC signature, Rust validation, frontend keys, query boundary, and exact-context cache writes must land together. Removing active-state rejection, parent-owned stale guards, cancellation, and discard-only test infrastructure keeps the packet direct.

**Out of scope:**

- Cancellation or AbortSignal wrappers.
- Requiring a valid captured save context to remain active until an operation finishes.
- Discarding a completed A result from A's own cache after the UI switches to B.
- Parent-owned current-context refs, mutation settlement guards, or global pending helpers.
- Toggle rendering, tactic labels, synthetic cells, interleaving, header-X recompaction, and widths; Commit 7 owns these.
- New IPC commands, schema changes, or scorer changes beyond Commit 5.

**Implementation packet:**

**Files and responsibilities:**

- `src-tauri/src/features/snapshot/service.rs` — extend the existing `pub(crate) fn ensure_save_context(tx: &Transaction<'_>, context: &SaveContext) -> Result<(), String>` used by snapshot ingest and staff assignment targets. First check whether `saves.id` exists and return `Save {id} not found` when absent. Then check the same row's `context_token` and return `Save changed or no longer exists` when it does not match. Retain the existing `SaveContext` signature, update affected callers/tests for the distinct unknown-ID error, and do not include `is_active` in this validator.
- `src-tauri/src/features/planner/tactic.rs` — preserve trusted `get_tactic(&Connection, save_id)` and `save_tactic(&Connection, save_id, &tactic)` callers. Add transaction-scoped `load_or_initialize_tactic_in_tx` and `save_tactic_in_tx` helpers so each command can validate context and read or write in one transaction. Keep one tactic validation before a save and no nested transaction.
- `src-tauri/src/features/planner/commands.rs` — change `get_planner_tactic`, `get_planner_tactic_options`, and `save_planner_tactic` to accept exact camelCase `{ saveId, contextToken }` plus `tactic` for save. Each command acquires the DB lock, opens one transaction, calls `ensure_save_context`, performs its read/options/save work for `save_id`, and commits once. Remove implicit `active_save_id` lookup. A valid inactive A context remains writable; an unknown or token-mismatched context fails before a read/default insert/write.
- `src/features/planner/api/fetch-planner-tactic.ts`, `fetch-planner-tactic-options.ts`, and `save-planner-tactic.ts` — require captured `{ saveId, contextToken }` and pass exact camelCase IPC arguments. Remove custom abort handling.
- `src/features/planner/api/planner-keys.ts` — key tactic and options by the full captured context. `{1,"tokenA"}` and `{1,"tokenB"}` must be distinct.
- `src/features/planner/api/planner-tactic-query-options.ts` and `planner-tactic-options-query-options.ts` — accept non-null context only. Do not create sentinel keys, nullable factories, or conditional hooks.
- `src/features/planner/components/tactic-context-boundary.tsx` — own the two unconditional queries for one non-null context. Expose typed `tactic`, `options`, `isPending`, `initialError`, `refreshError`, `readOnly`, and `retryBoth`. Initial failure shows `Could not load tactic` and Retry refetches both keys. A refresh failure retains cached data read-only.
- `src/features/planner/components/planner-tactic-editor.tsx` — key/remount the editor by full context and send full context in the mutation variables and mutation key. On success, write only `plannerKeys.tactic(vars)` for the originating context. Do not look up the active save and do not redirect the result to a current-context key. The old editor unmount isolates draft and feedback state when the UI switches context; no parent-owned stale guard is required.
- `src/app/routes/my-club.tsx` — derive the active captured context from `SaveSummary`, mount one `TacticContextBoundary` only for a matched settled snapshot, and key the editor by full context. Preserve pending, initial error with Retry, cached refresh error read-only, and existing snapshot/saves/Define DNA behavior.
- `src/features/search/utils/dynamic-columns.ts` and `src/app/routes/search.tsx` — admit a valid tactic sort only when its exact ID is present in the current table layout. Invalid, removed, or wrong-table IDs fall back to `defaultSearchSort(view)`. Keep tabs and `SearchFilterBar` mounted when the snapshot is null or mismatched. Search does not mount `TacticContextBoundary` until Commit 7, so it performs zero tactic IPC in Commit 6.
- `src/testing/planner-ipc-mock.ts`, `src/testing/setup.ts`, and `e2e/tauri-ipc-stub.ts` — model known save contexts by `{ saveId, contextToken }`, not one global active tactic. Multiple existing saves may remain valid while one is inactive. Reads and saves use context-keyed stores and counters; a valid inactive context succeeds, an unknown ID returns `Save {id} not found`, and a token mismatch returns `Save changed or no longer exists`. Saving A must round-trip through later A reads without changing B. Validate exact camelCase shapes.
- `src/lib/with-abort.ts` — verify that this absent file stays absent and that no references are introduced.

**Behavior and data flow:**

- A user clicks Save tactic while A is active. The frontend captures A's `{ saveId, contextToken }` in the mutation variables. Rust validates that exact row and token inside the command transaction, then writes A. If B becomes active before the command acquires the lock, A still completes because A remains the captured target. B is never consulted.
- If save ID 1 is deleted and recreated with a new token before an old command starts its transaction, the old token fails with `Save changed or no longer exists`; it cannot write the recreated save.
- A delayed read or save for A may populate `plannerKeys.tactic(contextA)`. The current B view reads only `plannerKeys.tactic(contextB)`, so A cannot overwrite B's tactic or labels. Switching back to A may reuse A's completed cache entry.
- Query factories receive only non-null context. `TacticContextBoundary` owns pending/error/refetch state. My Club mounts it for a matched settled snapshot; Search defers the mount to Commit 7.
- A tactic URL sort is accepted only when that exact synthetic ID is valid and present in the active view's persisted table layout. No-snapshot Search keeps its tabs and filter strip while the results panel shows the existing empty state.

**Ordered implementation steps:**

1. Replace active-only validation with `ensure_save_context` and complete the one-transaction Planner command helpers. Prove valid inactive A access, unknown ID rejection, token-mismatch rejection, no write on rejection, and successful commit.
2. Update all frontend IPC callers, keys, query factories, mocks, and E2E stubs to full captured context. Remove abort, sentinel, global pending, active-only mock, and parent-owned stale-settlement helpers.
3. Keep one My Club `TacticContextBoundary`, key the editor by full context, and write save results only to the mutation variables' context key.
4. Add Search layout-aware tactic sort validation and keep tabs/filter controls mounted for null or mismatched snapshots without mounting tactic queries.
5. Add focused regression tests, run packet validation, and obtain a fresh read-only review.

**Tests and proof:**

- Rust command-path tests: active A and inactive A with the same valid token can read options/tactic and save; unknown ID returns `Save {id} not found`; existing ID with the wrong token returns `Save changed or no longer exists`; rejected and invalid tactic saves leave persisted A unchanged; successful save commits and later A read returns it.
- Frontend IPC/key tests: exact camelCase arguments include `saveId` and `contextToken`; query and mutation keys distinguish A, B, and same numeric ID with a new token; no zero-argument or sentinel factory remains.
- Delayed A save test: start A, activate B before resolution, then resolve A. A's context-keyed persisted/mock value and A cache contain the saved tactic; B's value/cache remain unchanged; the mounted B editor shows no A feedback or draft. This proves safe targeting, not cancellation.
- Same-ID recreation test: an old-token command cannot write the new save; a new-token command succeeds and uses a distinct cache key.
- Boundary tests: tactic-only and options-only initial errors show Retry and refetch both keys; cached refetch failure preserves data read-only until recovery.
- Search tests: null and mismatched snapshots preserve tabs/filter controls, issue zero tactic/options/save IPC, and leave layout unchanged. Valid present tactic URL sort restores; absent, invalid, or wrong-table tactic sort falls back.
- Fixed-string proof: no `active_save_id` in the three tactic commands, no `ensure_active_save_context`, no `withAbortSignal`/`with-abort`, no parent `currentContextRef` tactic settlement guard, no zero-argument tactic key, no `?? -1`, no sentinel key/data, and no conditional query hook.

**Patterns to verify:**

- Reuse the `ClubDnaContext` and context-keyed query pattern, but use the narrower tactic contract that validates identity without active state.
- Follow existing snapshot transaction ownership: the command holds the DB lock and one transaction through validation and data access.
- Preserve the Search route's existing tab, filter, error, and default-sort composition.

**Constraints and non-goals:**

- Do not rotate `context_token` when a save becomes inactive; current `set_active_save_in_transaction` changes only `is_active` and `updated_at_utc`.
- Do not resolve the active save inside tactic commands or require `is_active=1` for captured-context access.
- Do not add cancellation, parent settlement refs, one-active-context mocks, or Commit 7 UI behavior.
- Do not weaken exact token validation or allow an old token to target a recreated numeric ID.

**Stop conditions:**

- Stop if repository behavior shows that activation changes `context_token`, because that would invalidate the captured-context contract.
- Stop if any tactic command still resolves `active_save_id`, if a save can be written without token validation, or if one command needs nested transactions.
- Stop if the simplified contract requires changing persisted schema, public save semantics outside tactic IPC, or Commit 5 scorer behavior.

**Review mandate:**

1. Every tactic IPC command uses the captured `save_id` and never resolves the current active save.
2. `ensure_save_context` checks row existence and token identity but not `is_active`.
3. One transaction owns context validation and read/default initialization/save; validation runs once before writes and success commits once.
4. A delayed A operation can update only A. It may finish after activation changes; B remains unchanged.
5. Same numeric ID with a new token cannot receive an old-token operation.
6. Query factories are non-null and hook-safe; initial and cached-refresh errors remain distinct and actionable.
7. Search sort and no-snapshot behavior stay within Commit 6; synthetic rendering and interactions remain in Commit 7.
8. Mocks and E2E stubs prove exact-context round trips instead of enforcing a single active context.

**Dependencies and sequencing:**

- Depends on Commit 3 for atomic layouts and Commit 5 for tactic query scoring. Commit 7 consumes the context-keyed reads and boundary but does not depend on active-only save rejection or stale-result discard.

**Validation:** `./scripts/dev check-app`, `./scripts/dev test src/features/search/utils/dynamic-columns.test.ts src/app/routes/search.test.tsx src/stores/use-player-table-store.test.ts src/app/routes/my-club-squad.test.tsx`, `./scripts/dev check-rust`, and `./scripts/dev smoke`. Run `git diff --check`, primary LSP diagnostics for all changed files, and the fixed-string proofs above. `search.test.tsx` and `my-club-squad.test.tsx` own the route/editor integration proof; no nonexistent test path may be used as a filter.

**Acceptance criteria:**

1. Tactic reads/options/saves target the captured `{ saveId, contextToken }`; changing the active save cannot redirect an operation.
2. A valid captured context stays valid when its save becomes inactive. Unknown and token-mismatched contexts fail before data changes.
3. Query/mutation keys isolate save contexts, including numeric ID reuse. A delayed A result may update A and never updates B.
4. My Club exposes actionable initial errors and read-only cached refresh errors through one typed boundary.
5. Search accepts only visible tactic sorts and retains tabs/filter controls with zero tactic IPC when no matching snapshot exists.
6. Commit 7 remains responsible for toggles, labels, synthetic rendering, interleaving, header-X behavior, widths, and all associated UI-state integration.
...
**Commit:** Captured save-context Planner tactic IPC, route sort gating, and no-snapshot composition

### RED or removal proof

Prove that tactic reads and saves use the context captured when the operation starts. A delayed A save that settles after B becomes active must update A and never B. Prove that an inactive but existing A context remains valid, while an unknown ID and a same-ID/new-token mismatch fail before any write. Also prove context-key isolation, Retry-both and cached read-only errors, visible-layout tactic sort validation, and Search tabs/filter controls with zero tactic IPC when no matching snapshot exists.

### Expected outcome

The existing Planner tactic IPC accepts exact `{ saveId, contextToken }` arguments and targets that row without consulting current active state. Query and mutation results are stored under the originating context key. My Club uses one typed tactic query boundary; Search defers that boundary to Commit 7 while retaining its no-snapshot composition and sort gate. No custom abort wrapper, global pending helper, active-only tactic mock, parent current-context ref, or stale-result discard path remains.

### Explicit exclusions

Toggle presentation, tactic labels and cells, interleaving, header-X recompaction, and widths remain in Commit 7. Commit 6 does not cancel a valid A operation merely because B becomes active.
...
- Commit 6 simplification after the second failed implementation cycle: the developer judged the human race window for a local tactic save too small to justify active-only rejection and complete stale-settlement suppression. The accepted replacement contract captures `{ saveId, contextToken }` when an operation starts, targets that exact save even if it becomes inactive, validates row existence plus token identity, and stores results only under the originating context key. This removes `is_active` from tactic context validation, parent-owned mutation settlement guards, discard-only cache assertions, and one-active-context mocks. Commit 7 keeps the same UI state model and context-keyed reads; its only test change is that delayed A may populate A's cache while B's labels/cache remain unchanged.
...
- `./scripts/dev test` — targeted `src/utils/tactic-ids.test.ts`, `src/features/search/utils/tactic-columns.test.ts`, `src/stores/use-player-table-store.test.ts`, `src/app/routes/search.test.tsx`, `src/app/routes/search.test.tsx`, plus `src/features/search/utils/dynamic-columns.test.ts` for sort validation; full suite at close-out.
