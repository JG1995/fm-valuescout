...
- 2026-07-29 (`/finish-feature` HIGH/MEDIUM fix — `743dcb4`): Removed unknown-version `26.3.0` fallback (fail closed). Dump + ready status now expose `scanTruncated` / `maxAccepted` (schema v5 amended in place). Rust validator requires those keys and never-null player structural fields. `Plugin.Unload` joins the poll thread. `loanListed` bit1 remains provisional (documented).
...
| 4 | Follow-up — finish-feature HIGH/MEDIUM remediation | `743dcb4` | Fail closed; scanTruncated contract; stronger validator; Unload join |
