...
- `Extraction/`: `AttributeScale` (÷5 → 1–20 or null; personality 1–20 or null), `PlayerAttributeReader` (three named `int?` maps).
- Dump schema **v3** player shape: `attributes`, `hiddenAttributes`, `personality` (stable PascalCase keys; JSON `null` for unread/invalid). Foot decode still uses `DecodeScaled` (0 allowed for comparisons).
- Diagnostics: encoding quirk lines + up to 5 `sampleAttributes` snapshots (Acceleration/Pace/Consistency/Ambition).
- Tests: attribute decode + null-sentinel + schema fixture updates. `dotnet test` 65 passed. `./scripts/dev check` green.
...
- 2026-07-29 (`/finish-feature` HIGH/MEDIUM fix — hash pending checkpoint commit): Removed unknown-version `26.3.0` fallback (fail closed). Dump + ready status now expose `scanTruncated` / `maxAccepted` (schema v5 amended in place). Rust validator requires those keys and never-null player structural fields. `Plugin.Unload` joins the poll thread. `loanListed` bit1 remains provisional (documented).
...
| 4 | Follow-up — finish-feature HIGH/MEDIUM remediation | Completed — hash pending checkpoint commit | Fail closed; scanTruncated contract; stronger validator; Unload join |
