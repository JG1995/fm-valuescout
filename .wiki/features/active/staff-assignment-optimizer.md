...
**Delivery fingerprint:** 3b0847dff51f540ee6d1ec8bb2e05ed246a7204166189852328c283d1854e6b0
...
- Team-scoped targets use the save's enabled Planner teams and current display names. Results and allocation use Senior, Reserves, and Youth in that canonical order. Manager targets exist only for enabled Reserves and Youth teams.
...
- IPC and command registration: Staff commands live in `src-tauri/src/features/staff/commands.rs`, use the shared `Db` mutex, resolve the active save in Rust, and are registered in `src-tauri/src/lib.rs`. Frontend Staff fetchers use `src/lib/tauri-client.ts` through `src/features/staff/api/` modules. The current-snapshot metadata DTO carries an immutable token, but `SnapshotSummary`, `get_current_snapshot`, and their frontend type currently omit it; Commit 4/5 must extend that existing contract before the optimizer can key freshness by token.
- Route and Query patterns: `my-club.tsx` loads current save, snapshot, managed club, tactic, and Planner depth with TanStack Query; its current Staff Shortlist context key and workspace props carry numeric save/snapshot IDs but no immutable snapshot token. It keys or suppresses other context-bound outcomes and invalidates feature query roots after mutations. The Shortlist workspace remains mounted inside an accessible hidden tab panel.
...
- Keep recommendation results ephemeral in a context-keyed mutation outcome. Persist target counts only.
...
- A stale result could remain visible during a pending save, snapshot, managed-club, Planner-team, or shortlist change if the route relies only on eventual context-key replacement, or survive the completed change if React or Rust keys freshness by reusable numeric snapshot ID instead of the immutable snapshot context token.
...
- The UI could duplicate Rust allocation logic or optimize only the currently filtered Shortlist page.
...
**Work:** Add the on-demand optimize action, context-safe result ownership, and accessible recommendation/vacancy evidence Panel.

**Size assessment:** About 360 changed non-test implementation lines. Exceeds the soft target because one coherent, trunk-safe recommendation outcome needs the typed token-guarded request/response, mutation state owner, setup/stale/error presentation, bounded semantic result table, and route reset wiring together; splitting would leave an unreachable result component or a mutation that can display stale recommendations.
...
- Extend Staff assignment types/API and the Shortlist component with `optimize_staff_assignments(expectedSaveContextToken, expectedSnapshotContextToken)`. Validate the returned immutable tokens against the current component context before presenting Rust-provided ready/setup/stale states, ordered bucket/slot rows, ScoreBadge values, Coach discipline, current/recruitment labels, vacancy evidence, and unsupported-job summary. `my-club.tsx` derives and passes `contextUnavailable` from its existing context query/mutation seams. `StaffAssignmentOptimizer` disables Optimize and immediately suppresses any visible recommendation while that prop is true. Reset results after target save or any route context-key change.

**Files and responsibilities:**

- `src/features/staff/types/staff-assignment.ts` — add explicit optimizer request/response context with save/snapshot IDs and immutable tokens plus state, recommendation, vacancy, evidence, classification, and Coach discipline DTO types.
...
- `src/features/staff/components/staff-assignment-optimizer.tsx::StaffAssignmentOptimizer` — accept the route-owned `contextUnavailable` prop; own configure/optimize controls, context-keyed mutation lifecycle, target-save reset, setup/stale messages, and result composition; disable Optimize and render no prior recommendation whenever context is unavailable.
- `src/features/staff/components/staff-assignment-results.tsx` — semantic ordered result table/Panel with accessible score names, explicit vacancies, classification, constraint explanation, unavailable counts, and unsupported Preferred Job note.
- `src/features/staff/components/staff-assignment-optimizer.test.tsx` — focused result, state, pending, error, reset, evidence, score, Coach, accessibility, and immediate `contextUnavailable` suppression tests.
...
- `src/app/routes/my-club-squad.test.tsx` — prove route-level immutable-context changes and Shortlist import suppress old and delayed results while filters do not affect the optimize request. Retarget the planned deferred route proof to render a recommendation, start the actual `PlannerTeamManagement` save flow with a deferred `save_planner_teams` command while old tokens and cached data remain present, and assert that the prior recommendation disappears and Optimize becomes disabled before the command resolves.
...
- Clicking Optimize sends only the expected save and snapshot immutable context tokens. Rust returns a bounded state/result with the resolved IDs and tokens. React shows pending text without changing control width and presents a result only when both returned tokens still match the mounted context. It never uses numeric snapshot ID as freshness authority and never sorts, maps jobs, selects scores, classifies staff, or fills vacancies. A stale/setup state replaces recommendations with truthful guidance. Before a context change settles, `my-club.tsx` observes its existing save/snapshot/managed-club/Planner refetch booleans and `playerResultContextMutationKey`; the Shortlist Modal reports its existing local import-pending transition through `onPendingChange`. Their combined `contextUnavailable` disables Optimize and suppresses the result even though the old token and cached data still exist. Saving targets or completing a successful save, snapshot, managed-club, Planner-team, or shortlist change then replaces the immutable context key or shortlist revision, so the prior outcome cannot return; a delayed result from a deleted snapshot remains suppressed even if SQLite reuses its ID.

**Ordered implementation steps:**

1. Add RED API and component tests for exact expected save/snapshot token arguments, each explicit Rust state, returned-token mismatch, pending/error phases, filled rows, vacancy rows, unavailable evidence, unsupported-job count, score labels, Coach discipline, and current/recruitment classification.
2. Add RED component coverage proving `contextUnavailable` disables Optimize and suppresses an already-visible recommendation immediately without discarding token response checks or target-save reset behavior. Add RED import-modal coverage for the existing local `pending` transition through `onPendingChange` while an import promise is deferred.
3. Add the typed optimize wrapper, minimum result Panel, `contextUnavailable` prop handling, and narrow Shortlist import pending callback to make focused tests GREEN.
4. Add RED reset and delayed-result tests for target save and every context-key input, including deletion plus forced reuse of the same numeric snapshot ID with a different token; add route tests proving shortlist presentation filters do not enter the optimize request.
5. Add the deferred route test: first render a recommendation, then use the actual Manage teams UI to start `PlannerTeamManagement`'s shared-keyed save mutation with `save_planner_teams` deferred while the old tokens and cached data remain present. Assert immediate result suppression and a disabled Optimize action before resolving the command; after successful resolution and key replacement, assert the old result does not return.
6. Wire the route key, token response check, shortlist revision, named refresh/mutation seams, and import-pending callback, then make suppression/reset proofs GREEN without effects that compute business rules or a global store.
7. Verify semantic table headings, accessible ScoreBadge names, keyboard reach, focus after Modal/action completion, bounded Shortlist/result scrolling, and the 1,750-row contract.

**Tests and proof:**

- RED: no Optimize action or result presentation exists; stale results cannot yet be suppressed.
- GREEN: focused tests fail if React trusts numeric snapshot ID, accepts a returned-token mismatch, leaves a recommendation visible or Optimize enabled while the actual Planner-team save command is unresolved, fails to surface the Shortlist Modal's immediate pending transition, invents order, omits vacancy/unavailable evidence, displays zero for missing score, hides classification/Coach discipline, sends filters, or retains results across immutable-context changes.
- Add: optimizer/result component tests and typed fixtures.
- Modify: route tests, `StaffShortlistImportModal` tests, and Staff IPC mock.
- Delete: none.
- Retain: Shortlist table presentation/filters/import, Staff Profile navigation, target Modal tests, and Planner team tests because they protect adjacent supported behavior.

**Patterns to verify:**

- Planner optimize mutation feedback, `PlannerTeamManagement`'s existing save mutation and local `onPendingChange`, `SquadOverviewPanel` and `isSquadResultBlocked` context suppression, the route's exact `isSnapshotRefreshing`, `isSavesRefreshing`, `isManagedClubRefreshing`, `isPlannerRefreshing`, and `useIsMutating({ mutationKey: playerResultContextMutationKey })` seams, `StaffShortlistImportModal`'s local `pending` plus callback props, `ScoreBadge`, `Panel`, `EmptyState`, semantic Data Table rules, and route-owned invalidation/reset patterns in `my-club.tsx`.

**Constraints and non-goals:**

- React renders DTOs only. Do not duplicate mapping, score maximum, sorting, allocation, classification, or vacancy logic. Do not persist results or put them in Zustand. Keep filters presentation-only. Use `—`, never `0`, for missing score. Preserve the existing table's route/profile behavior.
...
**Validation:** `./scripts/dev test src/features/staff/components/staff-assignment-optimizer.test.tsx src/features/staff/components/staff-assignment-target-modal.test.tsx src/features/staff/components/staff-shortlist-import-modal.test.tsx src/app/routes/my-club-squad.test.tsx` then `./scripts/dev check`

**Stop conditions:** Stop if the DTO lacks both resolved immutable tokens or enough evidence to explain a recommendation/vacancy without frontend inference, the existing route observer cannot see `PlannerTeamManagement`'s save mutation before `save_planner_teams` resolves, the Shortlist Modal's local pending state cannot report through one narrow callback, forced numeric ID reuse can display an old or delayed result, 1,750 rows break the bounded workspace, or accessible table/focus behavior requires a new shared primitive, route, or global store.

**Review mandate:** Verify no frontend business rules; exact save/snapshot immutable-token requests and response checks under forced numeric ID reuse; route-derived `contextUnavailable` uses the named existing refresh and `playerResultContextMutationKey` seams plus only the Shortlist Modal's narrow pending callback; the actual shared-keyed Planner-team save disables Optimize and suppresses visible results before `save_planner_teams` resolves; successful key/revision replacement prevents return; all setup/stale/error states plus target-save/context resets remain intact; filters stay excluded from requests; ordered semantic rows and bounded 1,750-row layout; score/Coach/current/recruitment evidence; vacancy/null/unsupported-job treatment; and Query/mutation ownership with focused component, import-modal, and retargeted deferred route proof that detects stale, delayed, or fabricated results.
...
- Playwright loads `/my-club?view=staff-shortlist` with the existing IPC stub, uses the visible Modal and buttons, and observes the semantic result table. The stub validates command shape and supplies deterministic Rust-like DTOs; it does not claim to prove SQLite or allocation.
...
2. Add the minimum contract-shaped stub responses and target-save state needed to exercise the existing real UI.
...
**Review mandate:** Verify only test/spec/stub paths changed; the stub remains contract-shaped with immutable-token guards and no business-rule duplication; smoke uses accessible selectors; the workflow covers configuration through results and same-ID/new-token reset; viewport/table behavior is meaningful; no Rust claim relies on the stub; test overlap is proportionate; and any product defect stopped and returned to its exact owning packet rather than entering Commit 7.

## Active work

**PR:** PR 1 — Optimize staff assignments from the Shortlist

**Commit:** Commit 5 — Configure assignment slots in My Club

### RED or removal proof

Add focused frontend tests that require snapshot and Load Data context tokens, reject equal numeric snapshot IDs with different tokens, exercise typed target IPC, and render an accessible complete target form in Staff Shortlist. Confirm they fail because frontend snapshot contracts and slot configuration do not exist.

### Expected outcome

Staff Shortlist exposes an accessible Configure slots Modal backed by typed target Query/mutation APIs. It renders Rust-provided team names and allowed pairs, submits one complete 0-through-50 entry per pair with immutable save context, and resets safely when route context tokens or other context inputs change.

### Explicit exclusions

Optimize action, recommendation results, frontend allocation or catalog invention, global state, new routes, optimizer browser commands, current-state documentation, and unrelated My Club changes.
...
`./scripts/dev test` must discover the new Staff assignment component, Shortlist import pending-signal, Load Data outcome, and route Vitest cases. `./scripts/dev check` must discover and pass the Rust target, migration, optimizer-query, and DTO tests in addition to Rust formatting, Clippy, Biome, TypeScript, and secretlint. Those Rust tests must include exact missing/extra/duplicate replacement rejection, composite-key uniqueness, forced rollback, stale save token, forced snapshot-ID-reuse/token-mismatch, and missing/null score unavailability proofs. The frontend Vitest suite must include equal-ID/different-token Load Data identity, same-ID/new-snapshot-token key, delayed-result suppression, and immediate suppression while the actual Planner-team save command is deferred. `./scripts/dev smoke` must discover the Playwright assignment workflow, but it proves only the browser route against the IPC stub; it does not prove Rust or SQLite.
