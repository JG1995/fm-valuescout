...
- The Tactic workspace uses one compact route header and one command bar for the phase view, save status when present, and **Save tactic** action. The pitches and selected-position settings shelf provide the current linked-position context without repeating it in the command bar.
...
- Tactic presentation: `src/features/planner/components/planner-tactic-editor.tsx` owns the draft, phase view, selected lane ID, linked highlight, validation, and save mutation. `planner-tactic-pitch.tsx` renders each pitch with current position-and-role buttons and linked counterpart emphasis, while `planner-tactic-inspector.tsx` renders one selected-position inspector. `planner-tactic-pitch.tsx` derives one shared three-to-five-slot density across IP and OOP, nests equal dark slot cards inside light base-position groups, and renders attacking bands above the defensive bands with GK last. `src/features/planner/utils/tactic-editor.ts` derives the same stable right-centre-left placement for labels and pitch columns regardless of role.
...
Commits 7 and 8 keep this ownership and replace only the pitch's derived geometry. Each phase groups lanes by base position without considering role, maps repeated central placements from stable tactic order, and uses the same mapping for visible and accessible qualifiers. Follow-up pitch polish derives one shared slot density from the densest visible row across both phases, clamps it to three through five slots, and nests equal dark slot cards inside light base-position groups. One placement uses centre; two use right then left; three use right, centre, then left. Additional existing placements continue through labelled overflow-row groups without changing or discarding tactic data. DOM and keyboard order follow the visible band and overflow-row sequence. Pitch row order renders the attacking bands first and GK last so both shapes attack upward. The position selector continues to expose the existing base placements only.
...
#### Follow-up — Adapt tactic pitch density

**Status:** Completed

**Provisional commit:** `fix(planner): adapt tactic cards to formation density`

**Work:** Replace one universal pitch card width with a shared adaptive density derived from the densest visible row across IP and OOP. Clamp the density to three through five slots, keep every dark tactical slot card equal within the current tactic, nest repeated central slots inside one light base-position group per visual row, centre populated wide cards within their light groups, and let empty outer position groups absorb unused width. Preserve stable right-centre-left tactic order, labelled overflow rows with visual row-major traversal, linked selection, phase controls, and tactic data.

**Implementation profile:** Luna Max — the change stays in React presentation but must coordinate shared cross-phase density, nested grid geometry, overflow rows, and existing linked-position behavior.

**Review profile:** Sol High — review must verify three-, four-, and five-slot densities, equal widths, non-overlap, group containment, empty-group scaling, accessibility, and unchanged tactic contracts.
...
- The developer chose to keep tactic-board geometry in PR 1 because it shares the existing Tactic review surface. Commits 6, 7, and 8 and the adaptive pitch follow-up are complete on the existing branch, so PR 1 is ready for publication.
...
- Developer-approved follow-up `e290b4c` removed the permanent linked-position summary and linked-position count from the command bar because the pitches and settings shelf already provide that context. This is an accepted presentation deviation from commit 9's original command-bar review mandate; it does not change tactic state, controls, validation, or save behavior.

## Completed work

| PR | Commit | Git ref | Implementation | Review | Deviations |
| --- | --- | --- | --- | --- | --- |
| PR 1 | Follow-up — Simplify the Tactic command bar | `e290b4c` | Removed the repeated linked-position summary and linked-position count so the command bar contains only the phase view, conditional save status, and Save tactic action. | Developer-approved direct UI polish; focused route coverage asserts the simplified hierarchy. | Accepted presentation deviation from commit 9's original command-bar contents; pitches and the settings shelf retain the current linked-position context. |
