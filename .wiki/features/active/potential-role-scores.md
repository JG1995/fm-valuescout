...
PR 1 implementation is complete and ready for publication. Feature validation, cross-commit review, documentation reconciliation, and final-publication readiness remain pending `$workflow-finish-feature`.
...
- Commit 6 validates the score basis at the command boundary and applies it only while constructing candidate lane scores. Potential candidates are loaded only from configured sources, projected once per candidate, then passed through the unchanged allocation, transaction, and provenance pipeline.
...
| PR 1 — Potential role scores | Commit 5 — Show projected visible attributes | `d78c24a` | One shared visible-attribute projection for role scoring and profile presentation, with accessible Current → Potential rows | Sol Medium — Accept | None |
| PR 1 — Potential role scores | Commit 6 — Optimize squads by potential | Pending record | Validated current/potential optimizer basis, source-scoped once-per-candidate potential scoring, shared allocation pipeline, and responsive mutually exclusive Planner actions | Sol High — Accept after 1 fix round | None |
