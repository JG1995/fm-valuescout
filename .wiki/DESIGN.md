...
- **Variants:** **No snapshot** — "No data loaded for this save", with Load Data as the action; this is the app's true first-run state and it must point at the one useful next step. **No results** — "No players match these filters", with "Clear filters". **Error** — a `body-md` phase-specific message, the underlying reason in `body-sm` `on-surface-variant`, and a Retry button. **Truncated** — results render normally with a persistent `warning` banner naming the cap.
