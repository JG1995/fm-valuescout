...
- **Development boosts:** a sibling Panel below Overview. **Boost CA** is the sole primary action and shows a snapshot-derived CA preview with clear disabled reasons. One Modal confirms the fixed boost and restores focus to its trigger. The panel reports the verified result inline and explains that FM may redistribute attributes over the next few in-game days. It has no numeric input or arbitrary value control.
