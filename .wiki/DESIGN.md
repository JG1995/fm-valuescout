...
- **Page header:** `Squad Planner` as `headline-lg`. The depth matrix adds **Senior** | **Reserves** | **Youth** team tabs. The tactic editor has **IP** | **OOP** | **Both** view tabs and the IP/OOP weight control.
...
- **IP/OOP weight:** one compact control above the tactic views, default `50 / 50`. It changes the combined score used throughout the planner and always shows both numeric weights. It is tactic state, not a per-player or per-team override.
- **Planned per-lane optimizer settings:** [Planner Optimizer Preferences](./features/active/planner-optimizer-preferences.md) replaces the one tactic-wide weight with settings owned by each linked lane. The selected lane gets one shared settings section outside the phase-specific controls, so **Both** view does not duplicate it. The section shows the lane's IP/OOP weight, optional unique priority, preferred foot (**Either**, **Left**, **Right**, or **Both**), and **Preferred** or **Strict** mode. Either means no foot restriction and disables the mode control. Controls use visible labels, native keyboard behavior, inline validation, and the existing failed-save draft retention.
...
- **Data honesty:** combined scores use the persisted IP and OOP role scores from the current snapshot with the tactic weight. Missing phase scores produce `—`, not a partial or zero score. A player outside the current club-family sources or absent from the snapshot remains assigned but carries a visible warning.
