...
Planned dedicated route `/academy`. The page tracks save-scoped youth cohorts against the club family already configured in Planner; it does not repeat club setup or import data from files.

- **Page header:** `Youth Academy` as `headline-lg`, the configured primary club as secondary context, and **Add class** as the primary action. The create Modal uses a labelled positive year input, prefilled from the current snapshot's in-game year when available, and previews `Class of YYYY` before submission.
...
- **Overview:** a compact KPI row shows Classes and Tracked players. Graduate, academy-income, released, goal, assist, and cap aggregates show `—` with an unavailable-memory explanation until their source fields exist. A reported Senior squad count may include only resolved members whose current snapshot explicitly says `team_level = senior`; its label or help text must make that limitation clear. Class cards are ordered newest year first and show the class year, tracked count, and only aggregates supported by current data.
...
- **Class workspace:** show the class title, tracked count, **Add players**, and destructive **Delete class** action. A compact semantic table contains name, age, nationality, positions, current club/reported team, PA, determination, height, preferred foot, appearances, goals, assists, caps, and sale fee. Unsupported or unknown values render `—`; do not substitute stars for PA or synthesize a personality label. The table owns horizontal overflow and preserves a readable Name column at 1280×800.
