...
- **Data honesty:** Current combined scores use persisted IP and OOP role scores from the current snapshot. Potential combined scores use projected visible attributes and the same lane IP/OOP roles and weight. Missing phase or projected-required attributes produce `—`, not a partial or zero score. Planner maps only exact `senior`, `reserve`, and `youth` team levels to its three teams. A player outside the managed club, at another team level, or absent from the snapshot remains assigned but carries a visible warning.
