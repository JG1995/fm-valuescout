...
- **Identity:** every shared analysis table has a required, caller-owned, non-removable sticky identity region before analysis columns. Player identity uses the name with available club and division context and reserves stable face and club-logo slots without loading images; staff supplies its own identity content through the same contract. Identity is separate from configurable analysis columns, so Club and Division are not configurable identity metrics and cannot be duplicated.
...
- **Compact strip (above results):** table-associated dataset toolbar content with active filter tags, an AND|OR mode indicator when more than one rule is set, **Clear all** when any rule is active, and **Edit filters** to open the modal. Staff Search adds Preferred Job and Only unemployed only while staff shortlist filtering is on. Route-owned page actions — **Add Tactic**, **Upload Shortlist**, **Configure Club Staff**, and **Optimize assignments** — remain in the page header outside the generic toolbar. Tags are `full` pills with `primary-container` fill, `on-primary-container` text, and a trailing remove button. Tag remove hover fills `surface-container-highest`.
...
- **Identity rail:** player name, club and division, age/DOB, nationality flags, height, preferred foot, and market value stay visible across sections. Each nationality flag exposes its name on hover and to assistive technology. Neutral portrait and crest placeholders reserve the identity slots. Transfer flags appear only when true. Overview's Ability region owns CA and concealment-gated PA; neither appears in the rail.
...
- **Development actions:** when information is revealed, **Boost CA** and **Wonderkid Mentality** sit inside the **Modify Player** disclosure in the shared analysis control band, separate from the hidden-information control. The shared action slot keeps one control-row minimum height and preserves overflow for downward-opening tooltips. Only the inline outcome uses a bounded vertical scroll region, so completed feedback remains reachable without moving the section switch. Hover or keyboard focus reveals each snapshot preview or disabled reason in a downward-opening tooltip that stays within the desktop viewport. One Modal confirms each action, preserves the existing guarded command contract, and restores focus to its trigger or the verified outcome. Results stay inline below the actions. There is no numeric input, arbitrary value control, or random-value selection.
...
- **Staff Search:** the shared configurable virtual table and filter editor cover the current snapshot. **Upload Shortlist**, **Configure Club Staff**, and **Optimize assignments** remain route-owned page-header controls outside the generic table toolbar. A URL-backed toggle beside the result count enables shortlist filtering. Preferred Job and Only unemployed appear only when it is on. Filtering off uses the `staff-search` layout and normal metrics. Filtering on retains the `staff-shortlist` layout and presentation: All jobs uses configurable CSV metadata; mapped jobs use fixed identity and metadata plus their mapped score and current sort; Coach shows six outfield coaching scores without automatic score sorting; unrecognized jobs use CA behavior. Upload, configuration, optimizer setup and result feedback, immutable context guards, Staff Profile navigation, and row navigation retain their existing behavior.
...
- **Page header:** `My Club` as `headline-lg`, the compact managed-club selector, supporting club context, and the current Club workspace share one wrapping header. The Planner board shows all enabled squads at once. The Tactic command bar has **IP** | **OOP** | **Both** view controls.
