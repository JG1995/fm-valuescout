...
- **Planned per-lane optimizer settings:** [Planner Optimizer Preferences](./features/active/planner-optimizer-preferences.md) adds optional unique priority, preferred foot (**Either**, **Left**, **Right**, or **Both**), and **Preferred** or **Strict** mode to the selected-lane section. Either means no foot restriction and disables the mode control. Controls use visible labels, native keyboard behavior, inline validation, and the existing failed-save draft retention.
