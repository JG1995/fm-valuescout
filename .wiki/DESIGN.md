...
- **Squad matrix:** tactic lanes are rows in stable pitch order. Ordered strings are columns labelled **1st String**, **2nd String**, and so on. The left lane column stays sticky; strings use a readable minimum width and horizontal scrolling rather than shrinking. Each lane label shows the IP position/role over the OOP position/role. Each occupied cell shows player name, current club or source team as secondary text, and one combined Score Badge. An unresolved player keeps the last-known name and replaces the score with `—` plus a warning label.
- **Player assignment:** activating an empty or occupied cell opens a 720px picker Modal. Candidates come from the configured club family. The default candidate set is the union of every source assigned to the target team, so a separate B or youth club is included automatically; the user can switch to **All club family**. Results show player name, source, IP score, OOP score, combined score, and any existing planner location. Sort by combined score descending by default. Selecting an unassigned player fills the cell. Selecting an assigned player opens a confirmation that names the old and new locations; confirming moves the player. Closing restores focus to the originating cell.
- **String management:** every team starts with one string and keeps at least one. A visible header overflow button and right-click on the same header open identical actions: **Add string after** and **Remove string**. The final remaining string disables Remove. Removing a populated string uses a destructive Modal that names the team, string, and assignment count. After removal, ordinal labels close the gap. The matrix header also has a visible **Add string** action so right-click is never required for discovery.
...
- **Accessibility:** pitches expose each lane as a labelled button with phase, position, and role in its accessible name. Lane selection and position changes work without drag. Team, mode, and phase controls use proper tab/segmented semantics. Matrix cells have row and column context in their accessible names. Header context menus follow the Level 4 `z-30` overlay rules, support Arrow keys and Escape, and return focus to their trigger.
...
- **Optimizer extensions** (order 7 in [TODO.md](./TODO.md)): automated assignment, formation comparison, best-and-worst candidate highlighting, gap recommendations, and Optimize controls on the planned pitch and matrix surfaces.
