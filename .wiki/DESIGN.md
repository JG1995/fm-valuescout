...
- **Analysis view switch:** a compact **General** | **Moneyball** tab strip sits below the shared player identity. General is the factory default, while Settings can make Moneyball the default when the profile URL has no view. A valid URL view wins. The outer switch preserves the inner General attribute `tab` URL value, so returning to General restores the selected attribute group.
