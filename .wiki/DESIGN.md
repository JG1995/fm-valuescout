...
- **Page header:** `My Club` as `headline-lg`, the managed club, and the **Squad** | **Planner** | **Tactic** tabs share one compact wrapping header. The depth matrix adds **Senior** | **Reserves** | **Youth** team tabs. The Tactic command bar has **IP** | **OOP** | **Both** view controls.
