...
- **Tactic editor:** one tactic per app save, shared by all teams. The editor starts from a 4-3-3 DM In-Possession shape linked to a 4-1-4-1 DM Out-of-Possession shape, with compatible general-purpose roles already selected. Each phase uses a top-down pitch with 11 numbered player lanes, arranged beside one selected-lane inspector. Selecting a lane opens its shared weight, rank, and foot settings plus the position and phase-compatible role controls for the visible phase or phases. The same lane number and connecting emphasis identify one player across IP and OOP. Both view places the two pitches side by side beside the inspector. Pointer dragging may move a lane, but selecting the lane and choosing a position must provide the complete keyboard path. Invalid or incomplete role-position pairs cannot be saved.
- **IP/OOP weight:** each linked lane has one compact control in the selected-lane inspector outside the phase-specific controls, so **Both** view does not duplicate it. Every lane defaults to `50 / 50`. Its control always shows both numeric weights and changes the combined score used throughout the Planner. It is lane state, not a player or team override.
- **Lane importance:** the selected-lane inspector has one optional rank from 1 through 11. Ranks are unique, gaps are valid, and the control uses a visible label and native select behavior. The optimizer handles ranked lanes in ascending order within the current team and string before it matches unranked lanes.
- **Preferred foot:** the selected-lane inspector has preferred foot (**Either**, **Left**, **Right**, or **Both**) and **Preferred** or **Strict** mode. Either means no foot restriction and disables the mode control. Controls use visible labels, native keyboard behavior, inline validation, and the existing failed-save draft retention.
