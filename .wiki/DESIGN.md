...
The Dashboard places **CSV enrichment** below the snapshot panels as a secondary action. It imports one supported Youth Tracker or Moneyball export into save-scoped enrichment for matching players in the active snapshot.
...
- **Idle:** explain that the user can import supported exported values for players already present in the current snapshot. Memory-backed identity and fields remain authoritative.
