...
> **Status:** Tokens, shared primitives (`src/components/ui/`, including **Modal** and **ScoreBadge**), the app shell (nav rail, top bar with **GlobalPlayerSearch**), the **Search** surface (compact filter strip, editor modal, virtualized results table), the **Player profile layout** (`/players/$uid` with Overview / Attributes / Roles tabs), and the **Squad Planner** club-family setup, dual-phase tactic editor, three-team depth matrix, Optimize control, and selected-team Clear Squad action (`/planner`) are implemented. `src/styles/global.css` bridges the full token set into Tailwind `@theme` ([ADR-0007](./decisions/0007-tailwind-css-v4.md)).
...
Dedicated route `/planner`. The planner shows club-family setup, the shared dual-phase tactic editor, and the selected team's depth chart in one working page. The route uses the active save and current snapshot shown in the global top bar.
...
- **Page header:** `Squad Planner` as `headline-lg`. The depth matrix adds **Senior** | **Reserves** | **Youth** team tabs. The tactic editor has **IP** | **OOP** | **Both** view tabs.
- **Tactic editor:** one tactic per app save, shared by all teams. The editor starts from a 4-3-3 DM In-Possession shape linked to a 4-1-4-1 DM Out-of-Possession shape, with compatible general-purpose roles already selected. Each phase uses a top-down pitch with 11 numbered player lanes. Selecting a lane opens its position and phase-compatible role controls beside the pitch. The same lane number and connecting emphasis identify one player across IP and OOP. Both view places the two pitches side by side. Pointer dragging may move a lane, but selecting the lane and choosing a position must provide the complete keyboard path. Invalid or incomplete role-position pairs cannot be saved.
- **IP/OOP weight:** each linked lane has one compact control in the selected-lane section outside the phase-specific controls, so **Both** view does not duplicate it. Every lane defaults to `50 / 50`. Its control always shows both numeric weights and changes the combined score used throughout the Planner. It is lane state, not a player or team override.
- **Lane importance:** the selected-lane section has one optional rank from 1 through 11. Ranks are unique, gaps are valid, and the control uses a visible label and native select behavior. The optimizer handles ranked lanes in ascending order within the current team and string before it matches unranked lanes.
- **Planned preferred-foot setting:** [Planner Optimizer Preferences](./features/active/planner-optimizer-preferences.md) adds preferred foot (**Either**, **Left**, **Right**, or **Both**) and **Preferred** or **Strict** mode to the selected-lane section. Either means no foot restriction and disables the mode control. Controls use visible labels, native keyboard behavior, inline validation, and the existing failed-save draft retention.
- **Squad matrix:** tactic lanes are rows in stable pitch order. Ordered strings are columns labelled **1st string**, **2nd string**, and so on. The left lane column stays sticky; strings use a readable minimum width and horizontal scrolling rather than shrinking. Each lane label shows the IP position/role over the OOP position/role. Each occupied cell shows the player name and one combined Score Badge. Unresolved and outside-pool assignments keep their occupied cell and show a warning; unknown scores render as `—`.
...
- **Squad actions:** **Optimize squads** is the primary action. It assigns eligible, unassigned players across all teams and strings using Rust-provided results, then reports pending, success, or failure without masking blank gaps. **Clear Squad** is destructive and applies only to the selected team after confirmation; it removes both manual and optimized assignments in that team. Both actions have keyboard paths and reconcile the displayed matrix and picker candidates after success.
...
- **Accessibility:** pitches expose each lane as a labelled button with phase, position, and role in its accessible name. Lane selection and position changes work without drag. Team and phase controls use proper tab semantics. Matrix cells have row and column context in their accessible names. Header menus support Escape and return focus to their trigger.
- **Data honesty:** combined scores use the persisted IP and OOP role scores from the current snapshot with the lane's IP/OOP weight. Missing phase scores produce `—`, not a partial or zero score. A player outside the current club-family sources or absent from the snapshot remains assigned but carries a visible warning.
