...
- **Identity rail:** player name, club and division, nationality flags, age, date of birth, height, preferred foot, and market value stay visible across sections in vertically aligned rows. A larger neutral stage reserves the portrait and crest slots without loading images. Transfer flags appear only when true. Overview's Ability region owns CA and concealment-gated PA; neither appears in the rail. The rail's bottom action area keeps **Modify Player** and the separate hidden-information control available across all four sections.
