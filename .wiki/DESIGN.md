...
- **Compact strip (above results):** table-associated dataset toolbar content with active filter tags, an AND|OR mode indicator when more than one rule is set, **Clear all** when any rule is active, and **Edit filters** to open the modal. Staff Search adds Preferred Job and Only unemployed only while staff shortlist filtering is on. Route-owned page actions — **Add Tactic**, **Upload Shortlist**, **Configure Club Staff**, and **Optimize assignments** — remain in the route-owned action row outside the generic toolbar. Tags are `full` pills with `primary-container` fill, `on-primary-container` text, and a trailing remove button. Tag remove hover fills `surface-container-highest`.
...
- **Staff Search:** the shared configurable virtual table and filter editor cover the current snapshot. The active top-navigation item identifies the destination, so the page does not repeat a **Staff Search** title. **Upload Shortlist**, **Configure Club Staff**, and **Optimize assignments** remain in a full-width route-owned action row outside the generic table toolbar; optimizer feedback wraps to the full page width below those controls. A URL-backed toggle beside the result count enables shortlist filtering. Preferred Job and Only unemployed appear only when it is on. Filtering off uses the `staff-search` layout and normal metrics. Filtering on retains the `staff-shortlist` layout and presentation: All jobs uses configurable CSV metadata; mapped jobs use fixed identity and metadata plus their mapped score and current sort; Coach shows six outfield coaching scores without automatic score sorting; unrecognized jobs use CA behavior. Upload, configuration, optimizer setup and result feedback, immutable context guards, Staff Profile navigation, and row navigation retain their existing behavior.
