...
- **Compact strip (above results):** table-associated dataset toolbar content with active filter tags, an AND|OR mode indicator when more than one rule is set, **Clear all** when any rule is active, and **Edit filters** to open the modal. Staff Search adds Preferred Job and Only unemployed only while staff shortlist filtering is on. Route-owned page actions — **Add Tactic**, **Upload Shortlist**, **Configure staffing needs**, and **Optimize assignments** — remain in the route-owned action row outside the generic toolbar. Tags are `full` pills with `primary-container` fill, `on-primary-container` text, and a trailing remove button. Tag remove hover fills `surface-container-highest`.
