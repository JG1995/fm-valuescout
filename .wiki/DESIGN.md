...
- **Attribute tabs:** the Attributes panel uses four tabs. Outfield players see **Outfield** | **Goalkeeping** | **Hidden** | **Personality**, defaulting to Outfield. Players with recorded GK familiarity of 15 or higher see **Goalkeeping** | **Outfield** | **Hidden** | **Personality**, defaulting to Goalkeeping. An explicit canonical `tab` URL search value wins; legacy `technical`, `mental`, and `physical` values normalize to Outfield. Missing or invalid values use the player-sensitive default. Arrow keys, Home, and End follow the visible order. Outfield renders Technical, Mental, and Physical together, with Set Pieces below Technical. For goalkeeper profiles, First Touch, Passing, and Technique move from Technical into the alphabetized Goalkeeping list.
