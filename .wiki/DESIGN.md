...
> **Status:** Tokens, shared primitives (`src/components/ui/`, including **Modal** and **ScoreBadge**), the app shell (nav rail, top bar with **GlobalPlayerSearch**), the **Settings** save, snapshot, and bridge management sections, the My Club managed-club selector, format-specific CSV enrichment in Squad, the minimal **Dashboard**, the **Player Search** surface (staged compact filter editor, categorized metric picker, full-height virtual table, persistent column layout, offline nationality flags, optional Moneyball role-score fields, and an automatic Moneyball-cohort **Shortlist** view with General metrics and independent layout), **Staff Search**, the managed-club **Staff** and save-owned **Staff Shortlist** workspaces, the **Staff Profile**, the **Player profile workspace** (`/players/$uid` with a compact summary, attribute tabs, and pitch-filtered General and Moneyball role fit), and the **My Club** workspace (`/my-club`, with `/planner` as a replace compatibility redirect) containing Squad, Planner, Tactic, Staff, and Staff Shortlist with its dual-phase tactic editor, full-height configurable Squad table, compact one-to-three-team depth matrix, save-scoped team management, Current/Potential score treatment, two Optimize actions, and one confirmed Clear all action are implemented. `src/styles/global.css` bridges the full token set into Tailwind `@theme` ([ADR-0007](./decisions/0007-tailwind-css-v4.md)).
...
Dedicated route `/staff` for standalone **Staff Search**, plus `/staff/$uid` for a Staff Profile. The canonical `/my-club` route owns URL-backed **Squad**, **Planner**, **Tactic**, **Staff**, and **Staff Shortlist** workspaces.
...
- **Staff:** shows every current-snapshot staff member whose club exactly matches the save's managed club. It has independent table layout and sort state, no search filters, and one confirmed **Boost all CA** action. Progress and completion copy preserve truthful updated, skipped, and failed counts; a failure after partial success states that already-applied FM changes remain.
- **Shortlist:** begins with **Upload CSV**, **Configure slots**, and **Optimize assignments** toolbar actions, followed by a labelled **Preferred Job** select and an **Only unemployed** checkbox. Configure slots renders FM26 **Coaching**, **Recruitment**, and **Medical** sections for each enabled squad. Club roles render once inside **Senior** when enabled, or once under standalone **Club**; this does not change their persisted scope. Set Piece Coach is club-wide. Assistant Manager remains separate from Coaches; Manager is excluded from Senior; Doctor and Chief Doctor are excluded. Head roles precede their ordinary counterparts. Club-wide lead and Set Piece Coach roles accept 0 or 1, while Scout, Recruitment Analyst, and squad count roles accept 0 through 50. It reports pending, setup, error, and success states. Optimize assignments is suppressed while context work is pending. Its result is a semantic bounded recommendation/vacancy table with Rust-provided team display names, Configure-slot ordering that integrates Club rows with Senior when available, score, typed Coach requirement, classification, and evidence; missing values render `—`. A panel-header chevron is accessible, retains the accepted result, hides only the body, does not rerun optimization, and resets expanded for a new result. Manage teams uses one confirmation for assignment and staffing-target removal impact. Before choosing a file, a replacement upload states that it replaces this save's active shortlist. The native picker accepts a staff export with the required columns; a success closes the modal and shows stored, total, and skipped counts beside the controls. A save or snapshot change closes the dialog and discards late picker or import feedback. All jobs uses its own configurable layout, with Preferred Job, Club Job, and Coaching Qualifications visible before role scores. A selected mapped job, including Manager, shows the fixed identity columns, qualifications, and one score sorted descending. Coach instead shows six outfield coaching scores without an automatic score sort. An unrecognized job shows no role score and sorts by CA. The raw Club Job value remains visible; blank and `-` both match **Only unemployed**. No shortlist, no-current-snapshot, missing-current-staff, and filter-empty states use distinct shared EmptyState copy.
...
- **Navigation and states:** activating a Staff Search, Staff, or Staff Shortlist row opens `/staff/$uid` and browser Back returns to the originating table URL. Exact legacy `/staff?view=my-staff` and `/staff?view=shortlist` links replace into the corresponding My Club workspace; `/staff/$uid` is never redirected. No snapshot, missing managed club, no shortlist, empty, unavailable-score, loading, and error states use the shared patterns without creating page-level nested scrolling.
...
Dedicated route `/my-club`; `/planner` is a replace compatibility redirect. The My Club route contains the Squad overview, managed-club Staff, save-owned Staff Shortlist, shared dual-phase tactic editor, and selected team's depth chart in five URL-backed workspaces: **Squad**, **Planner**, **Tactic**, **Staff**, and **Staff Shortlist**. Squad is the default for every save. Only the active workspace is visible, while all five remain mounted so local drafts, table layouts, and selections survive workspace changes. An explicit valid `view` search value wins. Workspace changes replace the URL search state rather than adding browser-history entries. The route uses the active save and current snapshot shown in the global top bar.

- **Managed club:** My Club shows one explicitly saved selector at the stable `/my-club#managed-club` target when a current snapshot exists. Its options are exact current-club names from the latest snapshot. The picker and **Save managed club** action share one responsive control row that wraps at narrow widths, while warning and error feedback remains below the row. A saved selection remains visible with a warning when a later snapshot no longer contains it; users replace it explicitly. Squad, Planner, Academy, and Staff derive membership from this one save-scoped selection. Staff Shortlist remains save-owned and is not managed-club filtered. No attached-club or fuzzy-name inference exists.
- **Page header:** `My Club` as `headline-lg`, the compact managed-club selector, supporting club context, and the **Squad** | **Planner** | **Tactic** | **Staff** | **Staff Shortlist** tabs share one wrapping header. The depth matrix adds **Senior** | **Reserves** | **Youth** team tabs. The Tactic command bar has **IP** | **OOP** | **Both** view controls.
