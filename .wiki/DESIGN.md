...
### Dashboard snapshot and save management

The Dashboard keeps recurring data actions and infrequent history management in one save-aware workspace. The top bar remains the only save switcher; the home route owns save creation, save rename/delete, snapshot history, and snapshot rename/delete.
...
- **Empty and loading states:** when a save has no snapshots, show `No snapshots stored` and direct the user to **Load Data**. The history panel keeps the existing Dashboard panel geometry and horizontal overflow behavior.
...
The Dashboard places **CSV enrichment** below the snapshot panels as a secondary action. It imports one supported Youth Tracker or Moneyball export for matching players in the active snapshot. Youth career values remain save-scoped; Moneyball values stay attached to that snapshot.

- **Header:** the panel title is `CSV enrichment`; the right-aligned **Import CSV** action uses the `secondary` Button variant so **Load Data** remains the Dashboard's primary data action.
- **No snapshot:** disable **Import CSV** and show the standard no-snapshot EmptyState with `No snapshot loaded` and `Load Data before importing a CSV export.`
- **Idle:** explain that the user can import supported exported values for players already present in the current snapshot. Memory-backed identity and fields remain authoritative; Youth career values stay with the save while Moneyball values stay with the snapshot.
- **Pending:** disable the action, show `Importing CSV…` on the button, and expose an `aria-live="polite"` status with the `Importing CSV` info chip.
- **Success:** show the detected format, parsed-player total, stored count, and skipped count. Explain that matching rows were stored or replaced and that unmatched exported IDs were skipped. Keep values bounded and do not show the selected path or raw player rows.
...
- **Club family:** Dashboard shows the setup panel after a snapshot is available, at the stable `/#club-setup` target. Choose a required primary club from a searchable snapshot-derived list. Every player at the primary club is eligible for Senior, Reserves, and Youth, regardless of the dump's `teamLevel`. Reserves and Youth each expose **Add associated club** for separately modeled B or youth teams; every player at an associated club joins that target team's pool. Do not suggest relationships from similar names. Missing configured clubs remain visible with a warning and a Replace action. Squad and Academy link to this panel when no club family is configured.
- **Page header:** `Squad` as `headline-lg`, the configured primary club, and the **Squad** | **Planner** | **Tactic** tabs share one compact wrapping header. The depth matrix adds **Senior** | **Reserves** | **Youth** team tabs. The Tactic command bar has **IP** | **OOP** | **Both** view controls.
- **Squad overview:** a configured family shows one paged, filter-free table of only its current-snapshot players. The fixed columns are **Name**, **Age / DOB**, **Nationality**, **Club**, **Division**, **CA**, **PA**, and **Value**; absent values render `—`. CA descending is the default. Every header is a sortable button and keeps its sort in the route URL; names are real player-profile links. One page holds 50 rows, so this table stays below the virtualization threshold while preserving arrow-key traversal between player links. An empty configured family explains that no current-snapshot players match.
- **Squad development:** the overview header has one primary **Boost all CA** action beside the secondary CSV uploads. Its confirmation explains the fixed age rule: +5 at age 20 or younger, +10 from age 21 through 28, and no boost from age 29. The action runs sequentially, prevents duplicate submission, and reports updated, skipped, and failed counts. If the app cannot verify a result or preserve the active context, it stops, disables the action, and tells the user to use Load Data before another boost. A newly current snapshot restores the action. It does not claim that skipped, failed, or recovery-stopped players changed.
...
- **Pitch geometry:** the editor starts from a 4-3-3 DM In-Possession shape linked to a 4-1-4-1 DM Out-of-Possession shape, with compatible general-purpose roles already selected. Each phase uses an attack-up pitch with the striker band at the top and the goalkeeper band at the bottom. Both pitches derive one shared card width from the densest visible row across IP and OOP, clamped to three through five slots, so every tactical slot has the same width within one tactic while different tactics can use different widths. A light position group contains its dark individual slot cards; repeated central slots share one group per visual row, later placements continue in labelled overflow-row groups, each populated wide slot is centred within its group, and empty outer groups absorb unused width. One central slot is centred; two form a centred pair; and three fill their group in right, centre, then left tactic order. Keyboard traversal follows the visible band and overflow-row order. Visible and accessible labels use football shorthand such as `MCR`, `MC`, and `MCL`; striker labels use `STCR`, `STC`, and `STCL`. Current IP/OOP positions and roles describe each selection; focus, hover, and selection emphasize the linked counterpart across both pitches without numeric markers.
...
- **Squad actions:** **Optimize squads** is the primary current-score action and **Optimize by potential** is a secondary explicit action. Both assign eligible, unassigned players across all teams and strings through the same Rust allocation and transaction path, then report basis-specific pending, one latest success, or failure without masking blank gaps. Only the score basis changes; source eligibility, ranking, matching, foot preferences, manual reservations, replacement, rollback, and assignment provenance stay shared. **Clear all** is destructive and requires confirmation that names Senior, Reserves, and Youth. One Rust transaction removes both manual and optimized assignments from every string in the active save while preserving strings, tactics, club-family settings, scores, and other saves. The actions stay in the shared toolbar in both matrix modes, prevent duplicate submission, retain errors, restore focus after close, and reconcile the displayed matrix and picker candidates after success.
- **States:** no snapshot uses the standard Load Data EmptyState. No club family shows Squad recovery guidance that links to Dashboard Club Setup. A configured source absent from the current snapshot shows a warning without deleting configuration or assignments. Loading skeletons match the matrix or pitch geometry. Tactic-save and assignment failures stay inline with the affected control and preserve the user's draft or prior assignment.
...
- **Data honesty:** Current combined scores use persisted IP and OOP role scores from the current snapshot. Potential combined scores use projected visible attributes and the same lane IP/OOP roles and weight. Missing phase or projected-required attributes produce `—`, not a partial or zero score. A player outside the current club-family sources or absent from the snapshot remains assigned but carries a visible warning.

### Youth academy layout

Dedicated route `/academy`. The page tracks save-scoped youth cohorts against the club family already configured in Planner; it does not repeat club setup or import data from files.

- **Page header:** `Youth Academy` as `headline-lg` and the configured primary club as secondary context. The Overview Classes panel owns the **Create class** primary action. The create Modal uses a labelled positive year input, prefilled from the current snapshot's in-game year when available, and previews `Class of YYYY` before submission.
...
- **Class workspace:** show the class title, tracked count, **Add players**, and destructive **Delete class** action. A compact semantic table contains name, age, nationality, positions, club, PA, determination, height, preferred foot, Apps, goals, assists, caps, Fee, and Actions. The Actions cell keeps labelled **Sell**, **Release**, and **Remove** controls visible, with success, warning, and destructive treatments; Release becomes **Restore** for an already released player, and the Sell edit dialog can restore a recorded sale. Unsupported or unknown values render `—`; do not substitute stars for PA or synthesize a personality label. The table owns horizontal overflow and preserves a readable Name column at 1280×800.
- **Player assignment:** **Add players** opens a searchable Modal restricted by the Rust service to current-snapshot players whose exact current club appears in the configured club family. Already classified players are absent. Rows show name, age, positions, and current club. Selection adds the player to this class; removal is available from the roster. Closing restores focus to the originating control.
- **Persistence states:** members who leave the club family or disappear from the current snapshot stay listed with their UID and last-known name. A visible text-and-icon warning distinguishes departed and unresolved records; neither condition deletes or reclassifies the player.
- **Empty and error states:** no snapshot uses the standard Load Data EmptyState. No club family points to `/#club-setup` and explains that Academy shares the Dashboard setup. A configured Academy with no classes offers Add class; an empty class offers Add players. Mutation errors stay with the triggering Modal or row and retain recoverable input.
