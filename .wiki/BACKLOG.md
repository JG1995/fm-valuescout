...
- **Historical Moneyball seasons** — CSV enrichment currently keeps one latest row per save, format, and player and has no season identity. **Trigger:** need to compare exported seasons, import waves, or trends. **Completion:** define an explicit season key and amendment rules, retain historical canonical statistics without changing current-row semantics, and add the required read model and UI.

- **Historical player development and snapshot comparison** — Snapshot history and management retains data but intentionally keeps Search, profiles, Planner, and Academy on the effective latest snapshot. **Trigger:** need to inspect a player's earlier state, compare snapshots, or chart development over time. **Completion:** add an explicit historical read model and snapshot-selection or comparison UX without changing latest-only default reads.
