...
| `staff` | Rows keyed by `(snapshot_id, uid)`, with stable scalar metadata and one `staff_attributes_json` object. They cascade when the owning snapshot is deleted. Bounded `search_staff` and `list_my_staff` commands read the effective current snapshot; Search includes every current row, while My Staff matches all configured Planner family clubs and reports a distinct missing-family state. No staff UI exists yet. |
