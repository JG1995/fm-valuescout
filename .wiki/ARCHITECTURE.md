...
Before a write, the plugin resolves the exact layout, reopens the live reader, and validates the UID and expected values. It then uses typed one- or two-byte writes with readback and rollback reporting. One gate serializes dumps and boosts. Optional status fields report only verified scalar results and never include an address or UID. The Overview tab exposes the fixed **Boost CA** action with a non-authoritative snapshot preview and confirmation. After verified success, its route invalidates snapshot, search, player, Planner, and Academy query roots. Wonderkid Mentality remains pending UI work under [ADR-0017](./decisions/0017-action-specific-fm26-player-boosts.md).
