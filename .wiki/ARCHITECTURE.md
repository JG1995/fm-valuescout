...
**Player search:** Rust `features/search` owns `search_players` and `suggest_players` against the active save's current snapshot. Search has General and Moneyball modes only. The grouped top navigation exposes Search and Moneyball as direct destinations at `/search?view=general|moneyball`; the route no longer renders a local view tablist. General can optionally join `player_shortlist_entries` by save and player UID before applying its existing filters, sorts, tactic columns, Club DNA, count, and paging. Moneyball remains a separate current-snapshot enrichment view and never controls or writes shortlist membership. React owns the `/search` route, URL-backed shortlist toggle, `Upload Shortlist` action, shared virtual table, filter editor, and global name search. A successful shortlist import invalidates the player search context and turns the General shortlist filter on. Legacy `/search?view=shortlist` URLs replace-normalize to General with the filter on. The General `search` layout serves both filtered and unfiltered rows; the former Player `shortlist` layout is retired. Profile analysis tabs remain local to profile routes.
