...
**Client UI state:** Zustand v5 (modals, layout chrome, selections not in the URL). `useMoneyballPreferences` persists the app-local Player Search default; the saved General value maps to Profile Overview and the saved Moneyball value maps to Profile Moneyball. Player and staff shortlist filter choices live in the non-persisted `useShortlistFilterStore`, keyed by surface and save context (id + context token), and last only for the session; the URL keeps only explicit `shortlistOnly=true|false` deep-link overrides. Stored shortlist presence is TanStack Query state from the typed search page `state`; shortlist membership remains save-owned and is not client UI state.
...
**Data:** SQLite via **rusqlite** (bundled) in Rust — migrations (`PRAGMA user_version`) and queries; schema v9 is the current dump contract, with migrations through v46; WebView never opens the database directly. Live FM26 dumps land on disk via the bridge file protocol (`%LOCALAPPDATA%\fm-valuescout\fm-bridge\`); **Load Data** is a native async Tauri command where the frontend hook captures the active save ID and context token and `api/load-data.ts` constructs the typed `Channel`; Rust verifies the supplied `saveId`/`contextToken` and echoes them on every progress event, validates preparation outside the database mutex, and publishes atomically on the fresh file `app-v2.db` (legacy `app.db` is never opened). Migrations v2–v43 cover `saves`, retained `snapshots`, `players` (`potential_attributes_json` plus current-only compact projection), `staff`, compact `player_role_metrics` (79 current + 79 potential nullable columns; 162 columns total) and `staff_role_metrics` (21 nullable columns) for the effective current snapshot only, immutable save/snapshot context tokens, optional snapshot names, shared snapshot-level boost recovery requirements, snapshot-owned Moneyball rows and cohort percentile scores, save-scoped Planner and Academy data, save-owned Youth career and player and staff shortlist rows, the save-owned v17 Moneyball quarantine, the save-owned `planner_teams` configuration, named `planner_strings`, save-scoped `staff_assignment_targets`, `managed_club_settings`, and the shared `saves.reveal_hidden_information` constrained boolean. Migration v42 clears only `planner_tactic_lanes`; the existing lazy `get_planner_tactic` seed recreates normal defaults per save, while planner assignments and all other save data remain intact. Migration v38 creates `player_role_metrics` and `staff_role_metrics` with the immutable 68/68/21 column inventory, model-version and score checks, and cascades; migration v39 drops the normalized `player_role_scores`, `player_potential_role_scores`, and `staff_role_scores` tables and indexes; migration v40 additively appends exactly 22 nullable checked generic OOP role columns (11 current + 11 potential) with no score backfill; migration v41 adds save-owned `player_shortlist_entries`. The catalog is 79 static FM26 roles. `SCORE_MODEL_VERSION=2` and `PROJECTION_MODEL_VERSION=2`. Migrated v1 rows remain physically preserved but compact-dependent current or potential reads reject them until a normal Load Data/current lifecycle writer materializes v2. Historical snapshots retain raw facts only; promotion rebuilds compact state. `app-v2.db` only; `app.db` remains untouched. Migration v29 preserves each prior primary-club selection while renaming its table and column, and drops obsolete attached-source rows; v30 adds nullable Moneyball percentile JSON without backfilling prior imports.

**FM26 bridge:** C# BepInEx 6 IL2CPP plugin in `bridge/` — memory layouts, safe block-based heap scanning (`TryReadBlock`), and `status.json` / `dump.json` / diagnostics with phase timings. On the live-validated exact FM 26.3.2 build, it exposes two bounded player-boost operations and one fixed +10, PA-capped staff CA operation through the local file protocol; the bridge owns all process-memory writes. Separate in-memory player and staff candidate indexes bind every action to its producing live scan, and one work gate serializes scans and mutations. Rust `features/memory_read` serializes requests for each bridge directory and sends only the closed boost requests. Rust owns sequential managed-club orchestration for Squad and Staff without adding bridge batch payloads. The closed `boost_managed_club` command holds the same mutation gate while it runs Wonderkid Mentality, age-restricted player CA, and staff CA in order from one live scan; it stops before the next phase when recovery becomes required. Rust `features/player` and `features/staff` derive snapshot-bound values, wait without the Db mutex, and reconcile each verified result into SQLite. React exposes guarded individual and managed-club boost controls. Rust `features/memory_read` also orchestrates dumps, validates dump shape, and installs the plugin DLL into Steam `BepInEx/plugins`; React `features/memory-read` shows install controls and bridge status. **Load Data** lives in `AppTopBar`. Windows Steam FM26 only. See [bridge/README.md](../bridge/README.md), [bridge scan performance](./features/completed/bridge-scan-performance.md), and [bridge-plugin-install](./features/completed/bridge-plugin-install.md).

**Snapshot ingest and management:** Rust `features/snapshot` owns save slots, transactional retained ingest from `dump.json`, the shared current-snapshot selector, metadata and save/snapshot management IPC, and token-bound destructive policy; React `features/snapshot` owns the save switcher, metadata-only snapshot overview, ordered history panel, and rename/delete/date-edit flows. The frontend supplies the invocation-captured active save ID and immutable context token to `load_data`; Rust acquires the load lease, verifies that context under a brief Db lock, runs the bridge scan without holding the Db mutex, captures a private copy only while `status.json` still identifies that ready request, then revalidates and ingests it with the request ID. Each retained snapshot persists its load timestamp, and Snapshot Overview and Snapshot History present that timestamp. Every successful ingest keeps prior rows, selects the greatest valid in-game date (then load timestamp and ID), and returns both stored and effective-current metadata. The token-bound `update_snapshot_game_date` command validates raw canonical `YYYY-MM-DD` input, updates only `game_date` in one transaction, reuses current selection and compact reconciliation, and skips Academy class writes. Its result reports updated metadata plus previous/current snapshot IDs. React awaits context cleanup before the command; winner changes invalidate route-owned current roots, same-current edits patch only the matching cached summary date, and non-current edits refresh history only. See [snapshot-history](./features/completed/snapshot-history.md) and [snapshot-date-edit](./features/completed/snapshot-date-edit.md).
...
**Shortlist imports:** `features/csv_import::player_shortlist` imports comma- or semicolon-delimited player CSVs with an exact `Unique ID` header. The 10,000-row limit counts data records yielded by the established CSV parser. It matches exact current-snapshot UIDs, deduplicates valid IDs, and atomically replaces migration v41's save-owned `player_shortlist_entries` only when at least one row matches. Moneyball imports never write this table. `features/csv_import::staff_shortlist` retains its semicolon-delimited `Unique ID`, `Preferred Job`, `Club Job`, and `Coaching Qualifications` contract and save-owned replacement semantics. Both imports use the 8 MiB trusted-file bound, parse outside the database lock, revalidate active save and current-snapshot context, reject zero-match replacement, and join stored UIDs to the effective current snapshot at read time. Core `search_players` and `search_staff` apply the optional shortlist restriction before normal filters, sorting, and paging. Staff Search also applies Preferred Job and unemployment predicates and returns saved CSV metadata through the shared shortlist composition; the dedicated staff shortlist reader and IPC command are retired. React owns context-bound picker flows, replacement warnings, summaries, the per-save-context shortlist toggle, and Staff presentation. A context change closes an import modal and suppresses its late result. See [Staff Shortlist CSV Enrichment](./features/completed/staff-shortlist.md).

**Role scoring:** Rust `features/scoring` owns a static FM26 IP/OOP catalog (79 roles), `score_role`, `score_role_unrounded` for comparison-local calculations, `combine_role_scores`, and the pure CA-to-PA visible-attribute projection. The projection returns current attributes unchanged for players aged 29 or older. The compact `features/player_metrics::compact` and `features/staff::scoring` writers persist one current-only projected visible-attribute JSON map on `players` alongside one `player_role_metrics` row (79 current + 79 potential) per current player and one `staff_role_metrics` row (21 roles) per current staff, using the v38 inventory plus additive v40 columns (`SCORE_MODEL_VERSION=2`, `PROJECTION_MODEL_VERSION=2`). Sparse source omissions become null after supplied visible-domain validation. The writer projects each player once and derives every role from that map. Ingest preparation occurs outside the Db mutex; the final transaction, current-snapshot selection and demotion, deletion promotion, and supported boosts own these writes atomically. Catalog, compact writer/ingest, Profile, Search resolver, Planner options/tactic lanes, and frontend display mirrors all consume all 79 roles. All 88 Moneyball presentation definitions map to known attribute roles. Tactic coverage is exactly 119/129 with ten intentional uncovered pairs: holding_wing_back_oop+DL/DR, pressing_wing_back_oop+DL/DR, box_to_box_midfielder_ip+MC, box_to_box_playmaker_ip+MC, deep_lying_playmaker_ip+MC, second_striker_ip+ST, wing_back_oop+DL/DR. See [role-scoring-engine](./features/completed/role-scoring-engine.md), [potential-role-scores](./features/completed/potential-role-scores.md), [completed record](./features/completed/ingest-potential-scores.md), [ADR-0026](./decisions/0026-eager-current-potential-scoring.md) (superseded), [ADR-0027](./decisions/0027-scoped-potential-read-validation.md) (partially superseded), and [ADR-0028](./decisions/0028-compact-current-snapshot-metrics.md).

**Player search:** Rust `features/search` owns `search_players` and `suggest_players` against the active save's current snapshot. Search has General and Moneyball modes only. The grouped top navigation exposes Search and Moneyball as direct destinations at `/search?view=general|moneyball`; the route no longer renders a local view tablist. General can optionally join `player_shortlist_entries` by save and player UID before applying its existing filters, sorts, tactic columns, Club DNA, count, and paging. A General shortlist-only probe page reports typed `state` (`no_shortlist` when the save stores no entries, `ready` otherwise), so the route can derive the toggle default from actual presence. Moneyball remains a separate current-snapshot enrichment view and never controls or writes shortlist membership. React owns the `/search` route, the per-save-context shortlist toggle (presence-derived default; explicit `shortlistOnly` URL values and legacy links override the session choice), `Upload Shortlist` action, shared virtual table, filter editor, and global name search. A successful shortlist import invalidates the player search context and forces the General shortlist filter on through the session choice. Legacy `/search?view=shortlist` URLs replace-normalize to General with the filter on. The General `search` layout serves both filtered and unfiltered rows; the former Player `shortlist` layout is retired. Profile analysis tabs remain local to profile routes.
...
**Planner tactic:** Rust `features/planner` owns eleven ordered, save-scoped `planner_tactic_lanes` rows. Each lane links compatible IP and OOP positions and roles, owns a 0–1 IP weight, one unique optional importance rank from 1 through 11, and a preferred-foot rule. Central placements store an explicit right, centre, or left value (`DCR` / `DC` / `DCL`, `DMCR` / `DM` / `DMCL`, `MCR` / `MC` / `MCL`, `AMCR` / `AMC` / `AMCL`, or `STCR` / `STC` / `STCL`); role validation and optimizer familiarity normalize each placement to its catalog base position. A qualified placement can appear only once per phase. Migration v42 clears stored tactic lanes once; `get_planner_tactic` lazily recreates the validated 4-3-3 DM In-Possession / 4-1-4-1 DM Out-of-Possession defaults per save. Planner assignments remain because they reference stable lane IDs, and unrelated save data is untouched. The Squad read path uses the assigned lane and its persisted IP/OOP roles and IP weight to derive Suggested Training at page-read time; it does not persist or cache the result. `get_planner_tactic_options` exposes catalog-backed placements and phase/position metadata; `save_planner_tactic` validates and replaces the complete lane set. React `features/planner` loads the tactic and options through TanStack Query. The editor composes one phase-aware normalized canvas with IP, OOP, and Both views, a persistent Tactical XI panel, and a persistent Selected Slot inspector. Pitch markers, panel rows, and inspector controls share one `selectedLaneId`; phase-local placement swaps preserve lane identity and non-placement settings, and clear incompatible roles for re-selection. The workspace uses portrait attack-up orientation below 2100px and landscape attack-right orientation at or above 2100px; the shared role-reference modal remains portrait and single-phase. All linked phase edits and selected-slot drafts remain local until save.

**Planner depth and optimizer:** Rust `features/planner` owns save-scoped `planner_teams`, named and ordered `planner_strings`, and `planner_assignments`. Migration v7 adds assignment provenance: existing rows and manual assign or move mutations are `manual`; optimized rows are `optimizer`. Migration v28 adds `planner_teams`: row presence makes the stable `senior`, `reserves`, or `youth` category available and stores its bounded display name. Migration v43 adds nonblank string display names with ordinal backfill and defaults. A save with no configuration initializes all three canonical rows and one string for each; later reads never recreate a removed category. `save_planner_teams` validates and atomically replaces the complete enabled-squad set and each squad's ordered named strings. Retained string IDs preserve assignments across rename and reorder; confirmed squad removal deletes only that squad's targets, strings, assignments, and team row, while confirmed string removal deletes only that string's assignments. Rust validates the full structure and removal scope before mutation. Restoring a category adds its canonical display name and one empty string with zero staffing targets. Player UIDs remain unique across a save, and assignments resolve against the active snapshot. A resolved assignment has current identity, a current combined score, and a potential combined score read from persisted lane-role scores and combined with the lane's IP weight; an outside-pool assignment still resolves but no longer matches the managed club; an unresolved assignment retains its last-known name when its UID is absent. Missing phase scores remain unknown. `optimize_planner_depth` accepts a validated `current` or `potential` score basis and runs one database transaction: it retains manual rows, removes earlier optimizer rows, then allocates eligible players from the shared exact-managed-club pool for available teams and ordered strings in canonical Senior, Reserves, Youth order. Existing category age limits still apply: Senior has no age limit, Reserves allows players through age 23, and Youth through age 18. Current candidates use persisted role scores; potential candidates use persisted potential role scores before both modes share ranked allocation, exact matching, foot handling, replacement, global player uniqueness, and rollback. `clear_planner_depth` requires confirmation and removes every assignment in the active save in one transaction. `get_planner_depth` returns available teams in canonical order with display names and named strings, while `get_planner_slot_candidates` remains current-score-only and ranks matching managed-club players by the Rust-computed lane-weighted combined score with any current assignment location.

React `features/planner` owns query, picker, confirmation, focus, and presentation state. React `features/squad` owns the Squad adapter for the shared player table and its typed IPC query layer. The grouped top navigation exposes Squad, Planner, and Tactic as direct Club destinations at `/my-club?view=squad|planner|tactic`. The `/my-club` route owns validated URL-backed `view` plus independent Squad sort state; page-level Club workspace tabs are removed. Planner and Tactic remain mounted with direct `hidden` props so local drafts and layout state survive destination changes. `/planner` replace-redirects to the equivalent My Club Planner URL with its sort mapping. The canonical route defaults to Squad and CA descending. When a snapshot exists, the route composes the single managed-club selector at `/my-club#managed-club` and invalidates the established Planner, Academy, and Staff roots after a successful selection. The Squad overview uses the same validated metric catalog and dynamic-value DTO as Search, keeps an independent persisted ordered column layout and widths, and requests bounded virtual pages through the shared full-height table. Its committed/requested Query observers retain rows only for sort replacement; route loaders do not own player-page result reads. It has no filters, but its sort may use any sortable metric, and the whole row opens the player profile. Unconfigured Squad, Academy, and managed-club Staff recovery links target the My Club selector. Settings retains current-context invalidation for save and snapshot changes, and its old managed-club anchor redirects to My Club. Top-navigation Club destination changes use normal Link navigation, add browser-history entries, and let browser Back return to the prior destination. Same-route changes preserve `squadSort` and `squadDir`; route-local sort controls use replace navigation. These navigation changes do not change Planner data or mutation ownership. The depth workspace renders one simultaneous board for every enabled squad. Its **Manage teams** Modal owns squad enable/name and string add/remove/rename/reorder, requires target-specific confirmation for populated removals, and reconciles depth and candidate caches after success. Both Optimize squads and Optimize by potential actions remain beside Manage teams and Clear all in the shared toolbar. Assigned cells show distinct accessible Current and Potential scores, while the two optimizer actions report basis-specific pending, success, and error states. The board keeps a sticky tactical-slot band with IP/OOP context, configured string names, compact fixed-width assignment cards, explicit Assign actions, and board-owned horizontal overflow from the 1280px minimum through ultrawide displays. Optimize and Clear all preserve their pending/error behavior and reconcile the returned depth cache and slot candidates, while the workspace exposes one latest successful-action status. Tactic saves invalidate both because roles and weight change their results. Load Data, active-save changes, and managed-club saves invalidate the entire Planner query tree. React displays Rust-provided unresolved, outside-pool, and unknown-score states without recomputing domain values. Squad boost commands accept a command-scoped typed Tauri channel and report Rust-derived cohort progress after each terminal player outcome; channel delivery remains best effort and the final result remains authoritative.

**Local FM graphics:** Rust owns a local-only graphics trust boundary. It persists one selected root in SQLite and performs bounded, no-follow discovery with root-relative mappings, deterministic precedence, and safe aggregate diagnostics. It performs no network access, download, copy, pack installation, or management. `GraphicsRuntime` rebuilds one selected root on a serial background worker, owns committed generations, in-memory indexes and raw image bytes, and enforces per-kind count- and byte-bounded caches. The asynchronous custom protocol is the sole raw-image path: the WebView requests `http://graphics.localhost/<generation>/<kind>/<uid>`, then Wry restores the registered `graphics://localhost/<generation>/<kind>/<uid>` URI before the Rust handler validates the closed generation/kind/positive-UID identity and returns image bytes; URI fragments are browser-side and are not sent to the handler. Settings choose, clear, and rescan operations invalidate generation-keyed frontend image queries. Player Profile, Search, Squad, and My Club use one lazy protocol image component with exact positive UIDs; fixed text and person/shield fallbacks remain available. Staff has no image presentation, and nullable UIDs never trigger name matching. Production limits are calibrated from a path-free representative-pack run; native-Windows custom-protocol/CSP proof remains an accepted gap.
...
│   ├── components/         # App-shell UI (AppShellLayout, AppNavBar, AppTopBar, not-found)
...
- **Client-only shared state** — Zustand (command palette, ephemeral multi-step UI before submit).
...
**Saves model** (migrations v2–v43, `src-tauri/src/db/migrations.rs`):
...
| `staff` | Rows keyed by `(snapshot_id, uid)`, with stable scalar metadata and one `staff_attributes_json` object. They cascade when the owning snapshot is deleted. Bounded `search_staff` and `list_my_staff` commands read the effective current snapshot; Staff Search and canonical My Staff expose configurable current-snapshot tables, managed-club Staff offers one fixed +10 PA/200-capped CA boost, and `/staff/$uid` presents the current Staff Profile with shared concealment and the individual fixed action. |
...
| `player_shortlist_entries` | One membership row per `(save_id, player_uid)`. Migration v41 stores one replacement list per save; rows survive snapshot replacement, cascade with the save, and join current players by UID at read time. |
...
| `planner_strings` | Ordered, named depth-chart strings for Senior, Reserves, and Youth. Rows reference the save, not a snapshot; each team keeps at least one string, and retained IDs preserve assignments across rename and reorder. |
...
User opens /my-club (legacy /planner replaces itself here)
  ...
  → React renders one simultaneous board for every enabled team, with a sticky tactical-slot band and board-owned horizontal overflow
  → save_planner_teams receives the complete enabled-team set and each ordered named string set; Rust validates it and atomically preserves retained IDs or scopes confirmed removals
  → get_planner_slot_candidates(team, laneId, search) returns Rust-ranked candidates from the exact managed club
  → clear_planner_assignment, assign_planner_player, and move_planner_player validate and mutate in Rust
...
`features/scoring/projection.rs` is a pure Rust projection module. It projects nullable visible attributes from CA to PA using natural-position groups and age factors, preserves nulls, rounds and caps values at 20, and applies the result through `score_role` and `combine_role_scores`. `score_role_unrounded` supports the Planner's comparison-local Suggested Training ranking without changing persisted or displayed role scores. `PA <= CA` and age 29 or older are identity projections. The compact writer validates supplied visible values in the 1–20 domain, normalizes sparse omissions to null, and stores a current-only projected JSON map on `players` alongside one `player_role_metrics` row per current player.
...
```text
User opens Search (top navigation or /search)
  → Search result controller: committed/requested Query observers for the first page
  → validateSearch normalizes view, shortlist filter, sort, dir, filters[], and combine in URL search params
...
search_players IPC (features/search/commands.rs)
  ...
  → optional shortlist restriction on General plus filters[] + filterCombine ("and" | "or") — max 32 rules
...
**Invariants:** `null` dump values never coerce to 0 for filter or display; role scores come from Rust and SQLite; every requested metric and sort ID is validated before SQL construction; requested fields remain bounded and deduplicated; and a potential filter or sort never runs against a partial required cohort. Player-table layout version 8 keeps Search at its Name, Age, Nationality, Height, CA, PA, and Value default while Squad appends its non-sortable Suggested Training column; Moneyball Search keeps its metric-focused default with Height after Nationality. The General `search` layout applies with shortlist filtering on or off. Applied filters, combine mode, and sort remain URL state; the shortlist filter is a session-only per-save-context choice whose default derives from stored presence, with explicit `shortlistOnly` URL values and legacy links overriding it. The filter editor keeps a local draft until Done; dismissing it does not apply the draft. Player Search view changes reset filters and sort, while legacy Shortlist URLs normalize to General with shortlist filtering on.

### 5.7.1 Squad overview table

The Squad workspace uses the same `VirtualizedPlayerTable`, `PlayerTableHeader`, metric catalog, and Rust `features/player_metrics` resolver as Search. Rust retains Squad ownership of exact managed-club membership and returns bounded `list_squad_players` pages with the requested dynamic fields plus a read-time `suggestedTraining` string or null. Explicit Planner assignment has priority; an unassigned developing player falls back to the tactic lane with the highest current two-phase tactic-adjusted score, using current IP/OOP role scores, lane `ip_weight`, both-position familiarity, preferred-foot rules, and tactic-order ties, then the existing training ranker filters to the player's exact age category before choosing the focus. `CA >= PA`, unknown age, age 29 or older, tacticless, incomputable, or no-eligible-lane rows return null. Only CA/PA- and age-eligible unassigned rows enter the one batched fallback-input load; ineligible rows bypass fallback-input loading and parsing. The fallback is read-only and page-read-time, with no persistence, cache, IPC, UI, or sort behavior added. The frontend renders only the focus or `—`, excludes this DTO field from `requestedFields`, and exposes no evidence or Details UI. Squad sort and direction remain validated in `/my-club` URL search state as `squadSort` and `squadDir`; the Suggested Training column is configurable but non-sortable, and filters are not added to the route. The eight default columns are only the initial layout. The per-table Zustand store persists Squad's visible metric IDs, order, and clamped widths separately from Search. Header sorting, pointer or keyboard resize, and Move left or Move right menu actions change presentation state without changing the requested metric set or resetting the virtual window. A row click or Enter opens the player profile, and Arrow Up/Down moves focus across page boundaries without exposing Previous or Next controls.
...
| Application UI with stubbed IPC: app shell (utility bar and grouped top navigation with Search, Moneyball, Staff Search, My Staff, Squad, Planner, Tactic, Youth, and Settings), bridge status, Search route, minimal Dashboard, Settings management sections, and Squad no-snapshot, first-use, CSV import, tactic, and Planner depth-board flows | Real `#[tauri::command]` handlers in Rust |
| User-visible navigation and form interaction in Chromium | SQLite persistence, migrations, or `app_data_dir` file I/O |
| Stub IPC for `get_bridge_status`, `get_bridge_install_status`, `install_bridge_plugin`, `remove_bridge_plugin`, `request_player_dump`, `list_saves`, `create_save`, `rename_save`, `set_active_save`, `list_snapshots`, `rename_snapshot`, `delete_snapshot`, `delete_save`, `get_current_snapshot`, `import_csv`, `search_players`, `suggest_players`, `get_player`, managed-club and Planner tactic commands, `get_planner_depth`, `save_planner_teams`, `optimize_planner_depth`, and `load_data` | Capabilities ACL, plugin permissions, native OS file dialogs, or menu/tray integration |
