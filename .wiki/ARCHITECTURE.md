...
**Player search:** Rust `features/search` owns `search_players` and `suggest_players` against the active save's current snapshot. Search has General and Moneyball modes only. General can optionally join `player_shortlist_entries` by save and player UID before applying its existing filters, sorts, tactic columns, Club DNA, count, and paging. Moneyball remains a separate current-snapshot enrichment view and never controls or writes shortlist membership. React owns the `/search` route, URL-backed shortlist toggle, `Upload Shortlist` action, shared virtual table, filter editor, and global name search. A successful shortlist import invalidates the player search context and turns the General shortlist filter on. Legacy `/search?view=shortlist` URLs replace-normalize to General with the filter on. The General `search` layout serves both filtered and unfiltered rows; the former Player `shortlist` layout is retired.
...
**Staff workspace and profiles:** Rust `features/staff` owns bounded core Staff Search, managed-club Staff reads, shared shortlist composition, assignment targets and optimization, and UID-only `get_staff`. Staff Search at `/staff` optionally joins `staff_shortlist_entries` by save and UID, applies Preferred Job, unemployment, and normal Staff filters before paging, and returns saved shortlist metadata. The former dedicated shortlist command and reader are retired. React `features/staff` owns Staff Search, shortlist upload, the URL-backed toggle, conditional Preferred Job and Only unemployed controls, `Configure Club Staff`, `Optimize assignments`, and `/staff/$uid`. Filtering off uses the `staff-search` layout and normal metrics; filtering on retains the persisted `staff-shortlist` layout and `staffShortlistPresentation(preferredJob)`. My Club retains managed-club Staff but no longer has a Staff Shortlist workspace. Legacy `/staff?view=shortlist` and `/my-club?view=staff-shortlist` URLs replace-normalize to `/staff` with filtering on. Assignment target, optimizer, context-guard, profile, and row-navigation semantics remain unchanged.
