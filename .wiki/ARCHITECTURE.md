...
Position suitability does not enter role scores. Planner tactic lanes persist the caller-supplied combined IP/OOP weights. Potential projection and role persistence are eager for effective current snapshots; do not add lazy fallback without a new decision. Full-matrix 184k-player ingest test is `#[ignore]`; gate keeps a 2k scored ingest timing check.

#### Potential role projection

`features/scoring/projection.rs` is a pure Rust projection module. It projects nullable visible attributes from CA to PA using natural-position groups and age factors, preserves nulls, rounds and caps values at 20, and applies the result through `score_role` and `combine_role_scores`. `PA <= CA` and age 29 or older are identity projections. The eager writer validates supplied visible values in the 1–20 domain, normalizes sparse omissions to null, and stores a complete current-only projected JSON map.

`features/player_metrics::potential_scores` calls the projection once per current player and derives every catalog role from that map. Migration v34 backfills each save's effective current snapshot. Ingest selection clears demoted snapshots and materializes a new winner, deletion promotion materializes the promoted snapshot, and supported boosts replace one player's projection and complete role set atomically. Search, Squad, Profile, and Planner read persisted values. A shared read-only invariant assertion runs before potential reads; Planner depth mutations run it before product-owned writes. Historical snapshots remain without potential-derived data.
...
search_players IPC (features/search/commands.rs)
  ...
  → features/player_metrics/resolver.rs: validate and deduplicate the closed metric catalog, build SQL expressions, and decode typed dynamicValues;
      role.* reads player_role_scores; potential_role.* reads exact-version persisted potential-role rows;
      attr./hidden./personality.* use json_extract; nationality uses json_each; position emits a stable strongest-first list
  → query.rs: assert the effective current snapshot when validated potential fields, filters, or sorts require potential data, then run read-only count, filter, sort, and page queries
...
get_player IPC (features/player/commands.rs)
  ...
  → query.rs: SELECT player scalars + JSON attribute maps for current snapshot;
      assert complete current potential data; SELECT persisted projected attributes and exact-version potential roles;
