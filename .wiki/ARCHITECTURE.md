...
**Saves model** (migrations v2–v39, `src-tauri/src/db/migrations.rs`):
...
ingest preparation (outside Db):
  ...
  → for each player: parse attributes_json → score_role per role (current 68)
      + projection (CA→PA) → potential 68; missing source → null; scale /20×100
...
`features/player_metrics::compact` and `features/staff::scoring` own the closed 68/68/21 mapping to the immutable v38 inventory. Current selection clears non-current compact rows and materializes the winner, deletion promotion rebuilds compact rows from retained raw facts before commit, and supported player boosts replace one player's projected attributes and compact current/potential columns atomically. Search, Squad, Profile, and Planner read named compact columns. Historical snapshots retain raw facts only.
...
get_player IPC (features/player/commands.rs)
  ...
  → query.rs: SELECT player scalars + JSON attribute maps for current snapshot;
      ...
      SELECT compact `player_role_metrics` row; map 68 current + 68 potential columns via closed catalog order
