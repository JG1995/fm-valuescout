...
**Data:** SQLite via **rusqlite** (bundled) in Rust — migrations (`PRAGMA user_version`) and queries; WebView never opens the database directly. Live FM26 dumps land on disk via the bridge file protocol (`%LOCALAPPDATA%\fm-valuescout\fm-bridge\`); **Load Data** captures, validates, and ingests the completed `dump.json` into the active app save as a retained snapshot, then selects the current row by the shared in-game-date ordering rule (migrations v2–v19: `saves`, retained `snapshots`, `players`, `staff`, `player_role_scores`, immutable save/snapshot context tokens, optional snapshot names, snapshot-owned Moneyball rows, save-scoped Planner and Academy data, save-owned Youth career rows, and the save-owned v17 Moneyball quarantine).
...
**Saves model** (migrations v2–v19, `src-tauri/src/db/migrations.rs`):
