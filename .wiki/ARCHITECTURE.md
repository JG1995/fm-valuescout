...
**Staff profiles:** Rust `features/staff` owns UID-only `get_staff` for the active save's effective current snapshot. It returns staff identity, employment and contract fields, the complete current attribute map, all 20 catalog-labelled current job-fit scores in stable catalog order, and the same save-scoped reveal preference used by `get_player`. React `features/staff` owns `/staff/$uid`, which presents the compact summary, Coaching/Mental/Knowledge current attributes, an internally virtualized catalog-ranked Role fit list, shared concealment control, and fixed +10 Boost CA action. Available job-fit values in the profile, Staff Search, and My Staff use the shared four-tier `ScoreBadge` ramp; missing scores remain neutral and never become zero. The command does not redact PA or any attribute; the profile owns presentation concealment and excludes player-only pitch, potential, and Wonderkid surfaces.
...
Cache invalidation: Load Data and set_active_save invalidate snapshot + search + player query keys (`playerKeys.all`). Snapshot history deletion invalidates current-only consumers only when the deleted row was current and a different row becomes effective current; active-save deletion or final-save recreation invalidates the full route-owned context tree. A successful supported player boost deletes the affected player's potential-role rows in the same Rust transaction as its snapshot update.
```

**Invariants:** `null` dump values never coerce to 0 for filter or display; role scores come from Rust and SQLite (not computed in the WebView); every requested metric and sort ID is validated before SQL construction; requested fields remain bounded and deduplicated; a potential filter or sort never runs against a partial required cohort; default layout columns start with Name, Age / DOB, Nationality, Club, Division, CA, PA, and Value, but users may add, remove, resize, and reorder sortable metrics independently per table. Applied filters, combine mode, and sort remain URL state. The filter editor keeps a local draft until Done; Cancel, close, Escape, and backdrop dismissal do not apply it, while Done adds each filtered metric once and then filters and columns remain independent.
