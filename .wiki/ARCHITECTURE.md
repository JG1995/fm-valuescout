...
**CSV preview and UID reconciliation:** Rust `features/csv_import` owns standards-compliant Youth Tracker and Moneyball parsing, fixture-backed header contracts, numeric UID validation, null-preserving normalization, bounded regular-file reads (1 MiB and 1,000 player rows), and safe errors. The `preview_csv_matches` command captures the active save, current snapshot, and snapshot UIDs under a brief Db lock, reads and parses the selected file outside that lock, then revalidates the same save and snapshot before returning exact match counts. Its DTO contains only the detected format and total, matched, and unmatched player counts; it never returns CSV rows, raw file contents, a machine-local path, or a persisted overlay. React `features/csv-import` uses the Tauri dialog plugin to request one `.csv` file, keeps preview state local, and renders the secondary **CSV reconciliation** Dashboard panel. Save switches and successful Load Data changes clear obsolete previews. The main capability adds only `dialog:allow-open` for the dialog plugin; the component requests a single non-directory CSV selection. Live memory-backed snapshot values remain authoritative, and this feature adds no migration or SQL write path. See [csv-player-reconciliation](./features/completed/csv-player-reconciliation.md).
