...
| `planner_tactic_lanes` | Eleven ordered, stable lanes per save. Each lane links an IP placement and role to an OOP placement and role, owns a 0–1 IP weight, and references the save directly. Both role references are validated against the scoring catalog. |
...
User opens /planner
  ...
  → save_planner_tactic receives the complete 11-lane draft with one IP weight per lane
  → Rust rejects incomplete lanes, unknown or phase-incompatible roles, unsupported positions, and lane weights outside 0–1
