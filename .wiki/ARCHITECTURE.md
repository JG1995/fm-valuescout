...
search_players IPC (features/search/commands.rs)
  ...
  → query.rs: materialize required potential roles for the complete current cohort before global filter/sort and only for the requested page for display-only fields; warm completeness uses exact-version counts and cold completion remains lazy
...
get_player IPC (features/player/commands.rs)
  ...
  → query.rs: SELECT player scalars + JSON attribute maps for current snapshot;
      project visible attributes once from CA, PA, age, and natural positions, returning current attributes unchanged at age 29+;
