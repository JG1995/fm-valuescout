...
**Player profiles:** Rust `features/player` owns `get_player` — one player by `uid` from the active save's current snapshot. It projects visible attributes once per request, merges persisted `player_role_scores` in catalog order, and returns current and potential role scores plus the projected visible-attribute map. React `features/player-profile` owns Overview / Attributes / Roles tab panels; Overview derives Best Role and Best Potential Role from those rows, Attributes shows visible Current → Potential values while Hidden and Personality stay current-only, and Roles shows both score bases. The route is `/players/$uid` with validated `tab` search param. Shared **ScoreBadge** (`table` / `card` / `hero` / `muted`) remains in `src/components/ui/score-badge/`. Search row activation and GlobalPlayerSearch navigate by route path only (no cross-feature imports). See [player-profiles](./features/completed/player-profiles.md) and [potential-role-scores](./features/completed/potential-role-scores.md).
