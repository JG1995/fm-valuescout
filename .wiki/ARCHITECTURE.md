...
**Local FM graphics:** Rust owns a local-only graphics trust boundary. It persists one selected root in SQLite (schema v9 identity contract with migrations through v46) and opens that root once with capability-owned, bounded, no-follow discovery. Discovery keeps root-relative mappings, deterministic precedence, safe aggregate diagnostics, and no path IPC. It performs no network access, download, copy, pack installation, or management. Rust candidate discovery and the native picker choose roots, while `GraphicsRuntime` owns committed generations, in-memory indexes and image bytes, and per-kind bounded available/missing caches. Settings choose, clear, and rescan operations invalidate the generation-keyed frontend image queries. Player Profile, Search, and Squad resolve portraits and current-club logos by exact positive UIDs; My Club resolves the stored exact managed-club UID. Staff has no image presentation. Fixed slots retain text identity and person/shield fallbacks, and logos remain decorative beside text. Nullable player, staff, manager, and managed-club UIDs preserve legacy null rows and never trigger name matching. The `rust-windows` job runs `./scripts/dev check-rust` and gates Check; GitHub CI status remains pending until its checks run.
