...
**Saves model** (migrations v2–v35, `src-tauri/src/db/migrations.rs`):
...
| `player_potential_role_scores` | Current-only derived role scores keyed by `(snapshot_id, uid, role_id)`, with nullable score and `projection_model_version`. Every effective-current player has one row per catalog role; historical non-current snapshots remain empty. Composite FK to `players` cascades on snapshot/player deletion, and an index supports exact-version validation, role filtering, and ordering. |
...
`features/player_metrics::potential_scores` calls the projection once per current player and derives every catalog role from that map. Migration v34 backfills each save's effective current snapshot. Ingest selection clears demoted snapshots and materializes a new winner, deletion promotion materializes the promoted snapshot, and supported boosts replace one player's projection and complete role set atomically. Search, Squad, Profile, and Planner read persisted values. Profile validates the requested player's projected map and complete role set. Search and Squad use indexed completeness counts for requested potential roles. Role reference validates its managed-club players and tactic roles, while direct depth reads validate assigned players' lane roles. Planner depth mutations retain the snapshot-wide preflight before product-owned writes. Historical snapshots remain without potential-derived data.
...
search_players IPC (features/search/commands.rs)
  ...
  → query.rs: validate exact-version rows for requested potential role IDs, then run read-only count, filter, sort, and page queries
...
get_player IPC (features/player/commands.rs)
  ...
  → query.rs: SELECT player scalars + JSON attribute maps for current snapshot;
      validate this player's complete projected map and exact-version potential role set;
