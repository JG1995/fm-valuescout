...
`test` runs `vitest run` (full suite or forwarded args). CI selects frontend, browser, Rust, and bridge product checks from the changed paths. Its required `check` status aggregates the applicable results. `package-windows` runs only on Windows, builds one unsigned x64 NSIS candidate from the locked source bridge, and writes its checksum under `.release/windows/<version>/`; it does not publish and is not part of `check`.
