...
import { createMockScoredPlayers, createMockScoredPlayer } from "./results-table.test-utils";
...
describe("ResultsTable", () => {
   describe("player rendering", () => {
      it("renders all players in the table", () => {
         ...
         for (const scoredPlayer of players) {
            expect(scoredPlayer.player).toBeDefined();
            expect(scoredPlayer.player.name).toBeTruthy();
         }
      });

      it("shows player name in table row", () => {
         const players = createMockScoredPlayers();
         expect(players[0].player.name).toBe("Lionel Messi");
      });

      it("shows player club in table row", () => {
         const players = createMockScoredPlayers();
         expect(players[0].player.club).toBe("Inter Miami");
      });
   });
   ...
   describe("sorting logic", () => {
      it("default sort is by valueAdjustedScore descending", () => {
         ...
         expect(sorted[0].player.name).toBe("Young Prospect");
      ...
      it("sorts by club ascending", () => {
         ...
         const sorted = [...players].sort((a, b) =>
            (a.player.club ?? '').localeCompare(b.player.club ?? '')
         );
         expect(sorted[0].player.club).toBe("Inter Miami");
         expect(sorted[1].player.club).toBe("Manchester City");
      });
   ...
   describe("row click behavior", () => {
      it("calls onPlayerClick with player id when row is clicked", () => {
         ...
         const players = createMockScoredPlayers();
         const firstPlayerId = players[0].player.id;
   ...
   describe("value formatting", () => {
      it("formats scores to 1 decimal place", () => {
         const player = createMockScoredPlayer({
            rawScore: 92.567,
