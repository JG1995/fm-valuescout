...
- `Extraction/` — dedicated readers for nested/indirect UTF-8 names, FM packed DOB, nationality, height, preferred foot (from foot attrs ÷5), and natural positions (suitability ≥ max(15, top−2)). Empty names or impossible DOBs are skipped and counted in diagnostics.
- Dump schema **v2** players: `{ uid, ca, pa, name, birthYear, birthDayOfYear, nationalities, heightCm, preferredFoot, positions }`.
...
4. Spot-check several known players’ UID/CA/PA plus name, DOB, nationality, height, foot, and positions. If wrong or empty, use diagnostics (class-offset histogram, sample UIDs, identity skip counts) to adjust `Fm263Layout`.
