...
4. The plugin polls every ~2s, rejects requests older than **30 seconds**, runs the dump off the Unity main thread, and updates `status.json` (`idle` → `scanning` → `ready` / `failed`). Transient status does not carry scan-cap or truncation signals; historical cap metadata remains in persisted dumps.
