...
Parse the CSV header row: strip BOM, detect delimiter, build case-insensitive column→index map, validate required columns exist. This is the first stage of every CSV parse.

## Files to Create/Modify

- Modify: `src-tauri/src/parser/headers.rs`

## Steps

- [ ] **Step 1: Write failing tests and implementation**

In `src-tauri/src/parser/headers.rs`:
...
pub fn parse_headers(header_raw: &str) -> Result<HeaderMap, String> {
    ...
    if original_names.len() < 3 {
        return Err(
            "File does not appear to be a valid Football Manager export.".to_string()
        );
    }
...
mod tests {
    ...
    fn sample_header() -> &'static str {
        "Unique ID;Player;Nation;2nd Nat;Club;Position;Age;Height;Left Foot;Right Foot;CA;PA;Transfer Value;Wage;Expires;Appearances;Minutes;Goals"
    }
    ...
    fn rejects_fewer_than_3_columns() {
        ...
        assert!(result.unwrap_err().contains("not a valid Football Manager export"));
    }
    ...
    #[test]
    fn all_80_columns_present() {
        // Build a header with all required columns
        let header = "Unique ID;Player;Nation;Position;Minutes;Goals;Assists";
        let result = parse_headers(header).unwrap();
        assert!(result.missing_required.is_empty());
    }
}
...
Run: `cd src-tauri && cargo test --lib parser::headers`
Expected: All 11 tests PASS.

## Dependencies

- Task 01 (types.rs must define `ColumnStatus`, `ParseWarning`)

## Success Criteria

- All 11 header parsing tests pass.
- BOM stripping, delimiter detection, case-insensitive mapping, required column validation all work.
- Files with < 3 columns are rejected.

## Tests

### Test 1-3: Delimiter detection for ;, comma, tab
**Feasibility:** ✅ Can be tested

### Test 4: Default semicolon on tie
**Feasibility:** ✅ Can be tested

### Test 5-6: BOM stripping
**Feasibility:** ✅ Can be tested

### Test 7-8: Header parsing and case-insensitive lookup
**Feasibility:** ✅ Can be tested

### Test 9: Rejection of files with < 3 columns
**Feasibility:** ✅ Can be tested

### Test 10: Missing required columns detection
**Feasibility:** ✅ Can be tested

### Test 11: BOM + headers combined
**Feasibility:** ✅ Can be tested
