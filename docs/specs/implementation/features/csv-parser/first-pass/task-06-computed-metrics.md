...
Implement per-90 value computation and ratio computation for all applicable stat fields. This runs as a post-extraction step on each `ParsedPlayer`, populating the per-90 and ratio fields in the stat category structs. Minutes = 0 or None → all per-90 values remain None. Division by zero → None (no error).

## Files to Create/Modify

- Modify: `src-tauri/src/parser/metrics.rs`

## Steps

- [ ] **Step 1: Write tests and implementation**

In `src-tauri/src/parser/metrics.rs`:
...
fn safe_add(a: Option<f64>, b: Option<f64>, c: Option<f64>) -> Option<f64> {
    let vals = [a, b, c].iter().filter_map(|&v| v);
    let mut sum = 0.0;
    let mut any = false;
    for v in vals {
        sum += v;
...
Run: `cd src-tauri && cargo test --lib parser::metrics`
Expected: All 9 tests PASS.

## Dependencies

- Task 01 (types: ParsedPlayer, all stat category structs)

## Success Criteria

- All 9 computed metric tests pass.
- Per-90 values computed correctly for normal minutes.
- Zero minutes → all per-90 None.
- None minutes → all per-90 None.
- Ratios handle zero denominator and None inputs correctly.
- Win rate computed from won/(won+drawn+lost).

## Tests

### Test 1: Per-90 normal values
**Feasibility:** ✅ Can be tested

### Test 2: Per-90 with zero minutes
**Feasibility:** ✅ Can be tested

### Test 3-5: Ratios (normal, zero denominator, None inputs)
**Feasibility:** ✅ Can be tested

### Test 6: Win rate
**Feasibility:** ✅ Can be tested

### Test 7: Aerial challenge rate
**Feasibility:** ✅ Can be tested

### Test 8: None minutes produce None per-90
**Feasibility:** ✅ Can be tested

### Test 9: Pressure completion rate
**Feasibility:** ✅ Can be tested
