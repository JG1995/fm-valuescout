...
Define all shared data structures used across the parser: enums (Role, Side, Footedness), core types (Position, Nationality, ParsedPlayer, ParseResult, SkippedRow, ParseWarning), and 8 stat-category structs that group the 80+ stat fields. These types form the contract every other parser module depends on.

## Files to Create/Modify

- Create: `src-tauri/src/parser/mod.rs` (module root, re-exports)
- Create: `src-tauri/src/parser/types.rs`
- Modify: `src-tauri/Cargo.toml` (add `csv`, `chrono`, `phf` dependencies)
...
- [ ] **Step 2: Create parser module root**
...
Create `src-tauri/src/parser/types.rs` with test module at bottom. Tests verify Serde round-trip and default behaviors.
...
mod tests {
    ...
    fn parse_result_serializable() {
        ...
        assert!(json.contains("Missing UID"));
    }

    #[test]
    fn nationality_with_and_without_code() {
        let nat1 = Nationality {
            code: Some("ENG".to_string()),
            name: "England".to_string(),
        };
        let nat2 = Nationality {
            code: None,
            name: "Ireland".to_string(),
        };
        assert!(nat1.code.is_some());
        assert!(nat2.code.is_none());
    }
}
```
...
Run: `cd src-tauri && cargo test --lib parser::types`
...
- `parser/types.rs` compiles and all 5 unit tests pass.
- `parser/mod.rs` declares all submodules (other files can be empty stubs for now).
- `Cargo.toml` has `csv`, `chrono`, `phf` dependencies.

## Tests

### Test 1: Serde round-trip for Position, Footedness
**What to test:** Serialize and deserialize enums and structs produce identical values.
**Feasibility:** ✅ Can be tested

### Test 2: ParsedPlayer::empty() produces valid skeleton
**What to test:** All required fields populated, all optional fields None.
**Feasibility:** ✅ Can be tested

### Test 3: ParseResult serialization
**What to test:** SkippedRow and ParseResult serialize to JSON correctly.
**Feasibility:** ✅ Can be tested

### Test 4: Nationality with/without code
**What to test:** Both variants compile and are usable.
**Feasibility:** ✅ Can be tested
